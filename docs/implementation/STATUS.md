...
## Current focus: stopped regression follow-up

The five benchmark prerequisites have working native paths, and the remaining
`SPEC_DEVIATIONS.md` backlog is open. Source `52b203037` is pushed and installed on
Linux x64 from new private archives. Required source checks, normal hooks, a clean
build and normal npm/fresh-Python installation passed. The previous `08b24f28a`
installation and all older versions remain preserved.

The old campaign stopped on a supported recovery-compaction defect at 144/180 LOW
...
The fix gives newly accepted native recovery its own captured-source epoch ACK
instead of reusing an older coverage boundary. Both native focused checks passed:
...
The new packaged DeepSeek LOW RPC check passed real recovery, the following accepted
MAIN request, manual compaction, a later normal prompt and EOF/exit 0. It used the
real packaged owners with HTTP substituted. Default refinement stayed enabled, but
no reviewer/planner request or completed refinement occurred in this short fixture.
...
current selection. Fresh host preparation also passed before provider admission.

The `52b203037` paired LOW campaign was stopped on 2026-09-12 at the user's new
quality gate: 167/180 primaries, 152 functional passes, 122 runtime-clean primaries,
15 completed retries and zero capacity invalidations. One cancelled current DeepSeek27
attempt has no finalized result. The controller, descendants and reporting heartbeat
are stopped. Results remain unchanged. **Vanilla is locked; MEDIUM remains cancelled.**

The active gate is now current-only: fix major functional failures first, then
runtime-clean and efficiency problems. Iterate failed current model/task pairs at LOW.
Only after problematic tasks pass cleanly, run all 30 current-only for regressions.
Other goal/release work is deferred until current is flawless and efficient across the
suite. Future comparisons must disclose the changed current policy and locked baseline.
OpenAI keeps the authorized ChatGPT subscription route; DeepSeek keeps its separately
authorized API credential. The earlier Sol13 wait's actual cause remains unknown.

Runtime `665f361da` implements useful explicit manual compaction on eligible short
sessions, a general implementation-contract prompt, and an SSE product default that
preserves explicit transport choices. Automatic compaction policy and the existing
Sol/Absolute Prohibition text are unchanged. Seven focused native cases and the
required repository check passed. Current-only host and model/task selection shipped
in `f6e30aa8f`; four focused runner checks, normal hooks and the clean build passed.

The current-only LOW iteration stopped on Sol6's confirmed runtime error:
`Context epoch view overlaps its literal tail`. DeepSeek5 and DeepSeek22 passed
and were runtime-clean with no retries. Sol6 failed and also timed out in solver
search and grading. Sol5 was cancelled unfinalized; Sol19 and Sol28 did not start.
All results remain retained. The controller and scoped descendants are stopped.
The source-order/tail boundary fix shipped in `7f6644a03`. The compiler chooses the
newest source reference rather than the last displayed tool result, and restores
tool-call order before public rendering.
Five focused native checks passed, including literal/public continuation after
reversed completion. Overlap/source/dependency checks remain strict. Two general
prompt rules address redundant successful checks and late search feasibility checks;
live accuracy and performance effects remain unproven. The trace places the runtime
failure after a later tool result, not in the successful manual compaction ACK.
...
`benchmarks/python-realworld-30/compare.py` remains the metric owner. Its paired view
includes the absent vanilla arm; current-only completion is shown in the current
rows, not the paired denominator. No reporting heartbeat is active. Runtime inputs
stay frozen during live timing. Full-suite improvement and release acceptance remain
unestablished; publication still requires separate final approval.
...
|---|---|---|
| Owned installation | The `52b203037` release archives installed on Linux x64 with normal npm scripts and fresh Python preparation; previous installations remain retained. | Other target platforms are not established by that installation. |
...
| Migration | Offline legacy-root import retains goals as history, keeps packages inactive and imports supported top-level schedules paused. Explicit recurring-cron resume requires a bound destination runtime. | No live-export guarantee, complete artifact/runtime remapping or implicit kernel/schedule restart. One-shot cron jobs need explicit rescheduling. |
...
| Distribution | New private archives and the branded installer template are available. | The final versioned asset set, benchmark review and separate publication approval remain required; nothing is published. |

### Release handoff: existing material and open deliverables

This is a navigation index, not release acceptance. Current status above and later
backlog dispositions supersede historical inventories.

- **Ancestry, identity and ports:** [product isolation](product-isolation.md),
  [historical H/D/S and port inventory](ports.md), [upstream adaptation](upstream-0.9.4.md),
  and [current deviations](../../SPEC_DEVIATIONS.md). Historical controls are not the
  fresh paired campaign or a claim that all old findings remain open.
- **Model policy and public interfaces:** [providers](../../packages/coding-agent/docs/providers.md),
  [DeepSeek text/tool scope](deepseek-v4.1-flash.md),
  [SDK](../../packages/coding-agent/docs/sdk.md), and [RPC](../../packages/coding-agent/docs/rpc.md).
  The current coverage table limits release claims; catalog entries alone do not expand support.
- **Installation, data and background work:** [quickstart](../../packages/coding-agent/docs/quickstart.md),
  [settings/update contract](../../packages/coding-agent/docs/settings.md),
  [supported offline import](../../packages/coding-agent/docs/sessions.md), and
  [background agents](../../packages/coding-agent/docs/long-running-agents.md).
  Binary rollback is not schema/data rollback. The [format guide](../../packages/coding-agent/docs/session-format.md)
  separates native framed/indexed storage and bounded owner reads from legacy JSONL
  import examples; native files are not editable JSONL transcripts.
- **Build and evaluation:** [development](../../packages/coding-agent/docs/development.md),
  the [source index](../../packages/coding-agent/.changes/base-context-source-index.md),
  and the [benchmark protocol/accounting](../../benchmarks/python-realworld-30/README.md).
  Current installed-path results are summarized above. Full LOW review, reproducibility
  and applicable G15/platform dispositions remain open; MEDIUM is cancelled.
- **Licensing and maintenance:** [LICENSE](../../LICENSE), [NOTICE](../../NOTICE),
  [security reporting](../../SECURITY.md), and [contribution policy](../../CONTRIBUTING.md).
  These do not supply the required JavaScript/Python dependency SBOM or establish
  complete subsystem/support ownership and maintenance coverage.

Before publication, finish the applicable tree/workspace/restart, aggregate hot-state,
migration and orphan-lifecycle dispositions; assemble the remaining required handoff
materials; finish matching versioned assets and the separately rendered `install.sh`;
and obtain final approval, npm grants and rights/visibility/version checks. No gap is
silently waived by this index, and no new validation suite is prescribed here.
...
Historical W31 gate: no live benchmark or readiness probe was allowed before all
five features worked. The user order at that point was all 30 isolated tasks at low
then medium effort, using the same session ChatGPT subscription. The later user
override cancels MEDIUM; the current paired LOW campaign is described above.
Publication still requires separate approval.
