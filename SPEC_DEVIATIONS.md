...
| **4.3 Authentication, endpoints, and local diagnostics** | O/P/A | Provider authentication is scoped to the selected provider. Remote telemetry, trace uploads/sharing, exporter credentials, and endpoint settings are removed in 1.0.1. Local diagnostics and request accounting remain. Installed-artifact qualification is pending for this release candidate. | CORE/auth-storage.ts; provider-contracts.ts; request-journal.ts |
