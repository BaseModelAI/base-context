...
const agentDirEnvName = "BASE_CONTEXT_HOME";
const startupBenchmarkEnvName = "BASE_CONTEXT_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific BASE_CONTEXT_HOME for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force BASE_CONTEXT_OFFLINE=1 / BASE_CONTEXT_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.BASE_CONTEXT_OFFLINE = "1";
		env.BASE_CONTEXT_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
