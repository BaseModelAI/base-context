...
from benchlib import (
    ...
    aggregate_sessions,
    apply_instrumented_accounting,
    apply_native_incurred_accounting,
    COST_KEYS,
    USAGE_KEYS,
...
def native_agent_end_error(event: dict[str, Any]) -> str | None:
    if event.get("type") != "agent_end":
        return None
    refusal = event.get("refusal")
    if isinstance(refusal, dict):
        if refusal.get("limit") == "value_encoding":
            detail = "Invocation output value cannot be represented as bounded JSON"
        else:
            limit = "message" if refusal.get("limit") == "messages" else "source byte"
            detail = f"Invocation output {limit} limit exceeded"
        return f"AgentError: {detail}"
    # Earlier failed responses can belong to a recovered request in this invocation.
    for message in reversed(event.get("messages") or []):
        if not isinstance(message, dict) or message.get("role") != "assistant":
            continue
        if message.get("stopReason") == "aborted":
            return f"AgentError: {message.get('errorMessage') or 'Request was aborted'}"
        return message_end_error({"message": message})
    return None


def native_prompt_completion_error(event: dict[str, Any], variant: str, package_name: str) -> str | None:
    if (variant, package_name) not in {
        ("current", NATIVE_PACKAGE), ("vanilla", "prime-agent"),
        ("vanilla", "@earendil-works/pi-coding-agent"),
    }:
        return None
    if (event.get("type") == "extension_error" and event.get("extensionPath") == "<session-input>"
            and event.get("event") == "prompt_completion" and isinstance(event.get("error"), str)):
        return event["error"]
    return None
...
def prepare_agent_home(run_dir: Path, args: argparse.Namespace, variant: str) -> dict[str, Path]:
    ...
    models = {"providers": {}} if variant == "current" else {}
...
def agent_command(
    ...
    options = {
        "variant": variant, "cwd": "/workspace" if variant == "current" else str(workspace),
        "agentDir": str(roots["config"]),
...
def isolated_agent_command(
    ...
    if host["kind"] == "native-base-context":
        result.extend(["--bind", cwd, "/workspace"])
        cwd = "/workspace"
    result.extend(["--chdir", cwd, "--", "/usr/bin/env", "-u", "PWD", *command])
    return result
...
def run_rpc(
    ...
    roots = prepare_agent_home(run_dir, args, variant)
    ...
    accounting_path = run_dir / "provider-cost-accounting.jsonl" if host.get("accounting_patch") else None
    if accounting_path is not None:
        # Provider-only: run_dir is mounted above; the solver launcher clears its environment.
        environment["PRIME_COST_ACCOUNTING_PATH"] = str(accounting_path)
    ...
    with stderr_path.open("w") as stderr, events_path.open("w") as event_log, transcript_path.open("w") as transcript:
        ...
        error: str | None = None
        stock_last_assistant_error: str | None = None
        stock_idle_observed = False
        ...
        try:
            ...
            while not done:
                ...
                if provider_capacity_error(event):
                    # Capacity invalidates this benchmark attempt, not the SDK's retry lifecycle.
                    rpc_capacity_observed = True
                prompt_error = native_prompt_completion_error(event, variant, host["package_name"])
                if prompt_error is not None:
                    error, done = prompt_error, True
                    continue
                if variant == "current":
                    terminal_error = native_agent_end_error(event)
                else:
                    # Stock publishes failed messages and agent_end before retry decisions.
                    message = event.get("message") or {}
                    if kind == "message_end" and message.get("role") == "assistant":
                        stock_last_assistant_error = (
                            f"AgentError: {message.get('errorMessage') or 'Request was aborted'}"
                            if message.get("stopReason") == "aborted" else message_end_error(event)
                        )
                    if kind == "auto_retry_end" and event.get("success") is False:
                        final_error = event.get("finalError")
                        error = (f"AgentError: {final_error}" if final_error else
                                 stock_last_assistant_error or "AgentError: Stock retry failed")
                    stock_idle_observed = kind == "benchmark_session_idle"
                    terminal_error = (error or stock_last_assistant_error) if stock_idle_observed else None
                    if kind == "benchmark_session_idle_error":
                        terminal_error = str(event.get("error") or "Stock SDK idle wait failed")
                ...
                if kind == "response":
                    ...
                    if not response["success"] and (variant == "current" or event.get("command") == "prompt"):
                        error, done = str(response["error"] or "RPC request failed"), True
                    elif response["id"] == compaction_request_id:
                ...
                elif kind == ("agent_end" if variant == "current" else "benchmark_session_idle") and not awaiting_compaction:
            ...
            while True:
                ...
                if trailing is not None:
                    ...
                    if event is not None and error is None:
                        error = native_prompt_completion_error(event, variant, host["package_name"])
                        if error is None:
                            error = (native_agent_end_error(event) if variant == "current"
                                     else message_end_error(event))
    ...
    invocation_finished = process.poll() is not None
    if accounting_path is not None:
        metrics = apply_instrumented_accounting(
            metrics, accounting_path, profiles, invocation_finished=invocation_finished,
        )
    elif variant == "current":
        metrics = apply_native_incurred_accounting(
            metrics, profiles, invocation_finished=invocation_finished,
            capture_qualified=(host.get("accounting_capture") or {}).get("qualified") is True,
        )
    metrics.update({
...
def parse_campaign_models(value: str) -> list[str]:
    labels = {part.strip() for part in value.split(",") if part.strip()}
    known = [model[0] for model in CAMPAIGN_MODELS]
    unknown = sorted(labels - set(known))
    if unknown:
        raise argparse.ArgumentTypeError(f"unknown campaign models: {unknown}")
    if not labels:
        raise argparse.ArgumentTypeError("no campaign models selected")
    return [label for label in known if label in labels]
...
def run_campaign(
    ...
    """Main owns real-run admission/host checks; schedule only selected models and tasks."""
    if not 1 <= args.max_workers <= 6:
        raise ValueError("--max-workers must be 1..6")
    task_ids = sorted(task_ids)
    models = [model for model in CAMPAIGN_MODELS if model[0] in args.models]
    ...
    campaign.update({
        ...
        "arm_order": "sequential selected arms in vanilla,current order; reverse when (task_index + model_index) is odd; model_index uses full sol,astra,deepseek order",
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        for phase in phases:
            results_by_model = {model[0]: [] for model in models}
            model_args = {}
            model_manifests = {}
            for label, provider, model, api, route in models:
            ...
            for offset in range(0, len(task_ids), 2):
                ...
                print(f"starting {phase} window for models {args.models}, tasks {window}", flush=True)
                futures = {}
                ...
                # All selected jobs finish before any next-window admission.
            ...
            for label, provider, model, _api, _route in models:
...
def apply_hosts_manifest(args: argparse.Namespace) -> dict[str, Any]:
    ...
    variants = parse_variants(args.variants)
    unknown = sorted(set(hosts) - set(VARIANTS))
    if unknown:
        raise ValueError(f"unknown hosts in manifest: {unknown}")
    missing = sorted(set(variants) - set(hosts))
    if missing:
        raise ValueError(f"hosts manifest missing selected variants: {missing}")
    for variant in variants:
        host = hosts[variant]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--three-model-campaign", action="store_true",
                        help="run selected tasks on Sol/Astra/DeepSeek, selected arms sequentially in two-task windows")
    parser.add_argument("--models", type=parse_campaign_models, default=",".join(model[0] for model in CAMPAIGN_MODELS),
                        help="comma-separated campaign model labels: sol,astra,deepseek (default: all three)")
...
def main() -> int:
    ...
    if args.three_model_campaign:
        if "vanilla" in variants:
            ...
            if len(task_ids) != 30 or sorted(task_ids) != sorted(scenarios):
                raise ValueError("--three-model-campaign with vanilla requires all 30 task IDs")
        task_ids = sorted(task_ids)
    ...
    if not args.three_model_campaign or any(label in args.models for label in ("sol", "astra")):
    ...
    if args.three_model_campaign and "deepseek" in args.models:
        if args.host_deepseek_api_key_file is None:
            raise ValueError("selected DeepSeek campaign model requires --host-deepseek-api-key-file")
    ...
    manifest = {
        ...
        **({"api_price_profiles": args.api_price_profiles} if args.api_price_profiles is not None else {}),
        ...
        "host_version": hosts_manifest["hosts"]["vanilla" if "vanilla" in variants else "current"]["version"],
