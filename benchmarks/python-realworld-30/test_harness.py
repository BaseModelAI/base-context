...
class HarnessComparisonTests(unittest.TestCase):
    ...
    def test_native_prompt_completion_marker_is_host_scoped_and_exact(self) -> None:
        event = {"type": "extension_error", "extensionPath": "<session-input>",
                 "event": "prompt_completion", "error": "real request preparation failure"}
        self.assertEqual(benchmark.native_prompt_completion_error(event, "current", benchmark.NATIVE_PACKAGE),
                         "real request preparation failure")
        for package in ("prime-agent", "@earendil-works/pi-coding-agent"):
            self.assertEqual(benchmark.native_prompt_completion_error(event, "vanilla", package),
                             "real request preparation failure")
        self.assertIsNone(benchmark.native_prompt_completion_error(event, "vanilla", benchmark.NATIVE_PACKAGE))
        self.assertIsNone(benchmark.native_prompt_completion_error(event, "current", "prime-agent"))
        for field, value in (("extensionPath", "some-extension"), ("event", "session_input")):
            self.assertIsNone(benchmark.native_prompt_completion_error({**event, field: value}, "current", benchmark.NATIVE_PACKAGE))

    def test_current_terminal_invocation_error_does_not_advance_stage(self) -> None:
        with tempfile.TemporaryDirectory(dir="/tmp") as directory:
            root = Path(directory)
            workspace = root / "workspace"
            workspace.mkdir()
            node = Path("/usr/bin/node")
            host = {"kind": "native-base-context", "package_name": benchmark.NATIVE_PACKAGE,
                    "package_root": str(root / "image/unpacked/base-context/package"),
                    "argv": [str(node), "/frozen/cli.js"]}
            args = argparse.Namespace(
                provider="openai-codex", host_deepseek_api_key_file=None,
                ...
                host_manifest={"node_executable": str(node), "dependency_root": str(root / "dependencies"),
                               "hosts": {"current": host}},
            )
            interrupted = {"role": "assistant", "stopReason": "error", "errorMessage": "connection interrupted"}
            terminal = {"role": "assistant", "stopReason": "error", "errorMessage": "provider rejected request"}
            events = [
                {"type": "message_end", "message": interrupted},
                {"type": "auto_retry_start", "attempt": 1},
                {"type": "message_end", "message": terminal},
                {"type": "agent_end", "messages": [interrupted, terminal]},
            ]
            process = Mock(pid=1235, stdout=io.StringIO("".join(json.dumps(event) + "\n" for event in events)))
            process.wait.return_value = process.poll.return_value = 0
            scenario = {"initial_prompt": "initial", "timeout_seconds": 10, "editable_paths": [],
                        "stages": [{"id": "initial", "compact_after": True},
                                   {"id": "followup", "message": "continue"}]}
            with patch.object(benchmark.subprocess, "Popen", return_value=process), \
                    patch.object(benchmark, "inject_stage") as inject, patch.object(benchmark, "stop_process"), \
                    patch.object(benchmark, "stop_attempt_processes"):
                observed = benchmark.run_rpc("current", root, scenario, workspace, root, args)
            self.assertEqual(observed["error"], "AgentError: provider rejected request")
            sent = [json.loads(call.args[0]) for call in process.stdin.write.call_args_list]
            self.assertEqual([item["type"] for item in sent], ["prompt"])
            self.assertEqual(observed["compaction_requests"], [])
            inject.assert_not_called()
            saved_events = [json.loads(line) for line in (root / "rpc-events.jsonl").read_text().splitlines()]
            self.assertEqual(saved_events, events)

    def _run_stock_events(self, events):
        with tempfile.TemporaryDirectory(dir="/tmp") as directory:
            root = Path(directory)
            workspace = root / "workspace"
            workspace.mkdir()
            node = Path("/usr/bin/node")
            host = {"kind": "public-prime-agent", "package_name": "prime-agent",
                    "package_root": str(root / "image/unpacked/prime-agent/package"),
                    "argv": [str(node), "/frozen/cli.js"]}
            args = argparse.Namespace(
                provider="openai-codex", host_deepseek_api_key_file=None,
                host_openai_codex_auth_file=root / "host-auth.json", model="gpt-5.6-sol", thinking="medium",
                bwrap="/usr/bin/bwrap", timeout_seconds=10, hosts={"vanilla": host},
                host_manifest={"node_executable": str(node), "dependency_root": str(root / "dependencies"),
                               "hosts": {"vanilla": host, "current": {
                                   "package_root": str(root / "image/unpacked/base-context/package")}}},
            )
            process = Mock(pid=1235, stdout=io.StringIO("".join(json.dumps(event) + "\n" for event in events)))
            process.wait.return_value = process.poll.return_value = 0
            process.sent_event_types = []
            def capture_send(_line):
                path = root / "rpc-events.jsonl"
                recorded = path.read_text().splitlines() if path.exists() else []
                process.sent_event_types.append([json.loads(line)["type"] for line in recorded])
            process.stdin.write.side_effect = capture_send
            scenario = {"initial_prompt": "initial", "timeout_seconds": 10, "editable_paths": [],
                        "stages": [{"id": "initial"}, {"id": "followup", "message": "continue"}]}
            with patch.object(benchmark.subprocess, "Popen", return_value=process), \
                    patch.object(benchmark, "inject_stage"), patch.object(benchmark, "stop_process"), \
                    patch.object(benchmark, "stop_attempt_processes"):
                observed = benchmark.run_rpc("vanilla", root, scenario, workspace, root, args)
            sent = [json.loads(call.args[0]) for call in process.stdin.write.call_args_list]
            return observed, sent, process

    def test_stock_retry_waits_for_sdk_idle_before_advancing(self) -> None:
        failed = {"role": "assistant", "stopReason": "error", "errorMessage": "Selected model is at capacity."}
        success = {"role": "assistant", "stopReason": "stop"}
        events = [
            {"type": "response", "command": "prompt", "id": "request-1-initial", "success": True},
            ...
            {"type": "agent_end", "messages": [success]},
            {"type": "benchmark_session_idle"},
            {"type": "message_end", "message": success},
            {"type": "agent_end", "messages": [success]},
            {"type": "benchmark_session_idle"},
        ]
        observed, sent, process = self._run_stock_events(events)
        self.assertIsNone(observed["error"])
        self.assertEqual([item["type"] for item in sent], ["prompt", "prompt"])
        self.assertEqual(len(observed["stage_events"]), 2)
        self.assertEqual(process.sent_event_types[1][-1], "benchmark_session_idle")
        self.assertTrue(observed["rpc_capacity_observed"])
        process.wait.assert_called_once_with(timeout=30)

    def test_stock_retry_start_rejection_is_terminal_only_at_idle(self) -> None:
        failed = {"role": "assistant", "stopReason": "error", "errorMessage": "temporary failure"}
        events = [
            {"type": "message_end", "message": failed},
            {"type": "agent_end", "messages": [failed]},
            {"type": "auto_retry_start", "attempt": 1},
            {"type": "auto_retry_end", "attempt": 1, "success": False, "finalError": "continuation rejected"},
            {"type": "benchmark_session_idle"},
        ]
        observed, sent, process = self._run_stock_events(events)
        self.assertEqual(observed["error"], "AgentError: continuation rejected")
        self.assertEqual([item["type"] for item in sent], ["prompt"])
        process.wait.assert_called_once_with(timeout=30)
    ...
    def test_current_win_uses_primary_and_native_physical_receipts(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir="/tmp") as directory:
            ...
            workspace.mkdir()
            node = Path("/usr/bin/node")
            host = {"kind": "native-base-context", "package_name": benchmark.NATIVE_PACKAGE, "package_root": str(root / "image/unpacked/base-context/package"),
                    "argv": [str(node), "/frozen/cli.js"]}
            args = argparse.Namespace(
                provider="openai-codex", host_deepseek_api_key_file=None,
                host_openai_codex_auth_file=root / "host-auth.json", model="gpt-5.6-sol", thinking="medium",
            ...
            interrupted = {"role": "assistant", "stopReason": "error", "errorMessage": "connection interrupted"}
            recovered = {"role": "assistant", "stopReason": "stop"}
            events = [
                {"type": "message_end", "message": interrupted},
                {"type": "turn_end", "message": interrupted, "toolResults": []},
                {"type": "auto_retry_start", "attempt": 1},
                {"type": "message_end", "message": recovered},
                {"type": "agent_end", "messages": [interrupted, recovered]},
                {"type": "compaction_end", "result": {"tokensBefore": 100}},
                {"type": "response", "id": "request-2-after-initial", "command": "compact", "success": True},
                {"type": "agent_end", "messages": [recovered]},
            ]
            ...
            saved_messages = [json.loads(line)["message"] for line in (root / "transcript.jsonl").read_text().splitlines()]
            self.assertEqual(saved_messages, [interrupted, recovered])
            # The same completion helper is used during drain; individual failed turns remain nonterminal.
            for event in events[:3]:
                self.assertIsNone(benchmark.native_agent_end_error(event))
            sent = [json.loads(call.args[0]) for call in process.stdin.write.call_args_list]
    ...
    def test_campaign_balances_windows_models_and_effort_phases(self) -> None:
        with tempfile.TemporaryDirectory(dir="/tmp") as directory:
            ...
            args = argparse.Namespace(output=root, max_workers=2, group_size=99,
                                      ...
                                      models=["sol", "astra", "deepseek"], api_price_profiles=[{"marks": []}])
    ...
    def test_current_only_low_campaign_retains_failure_without_admitting_other_arms_or_phases(self) -> None:
        with tempfile.TemporaryDirectory(dir="/tmp") as directory:
            ...
            args = benchmark.build_parser().parse_args([
                "--three-model-campaign", "--models", "deepseek,sol", "--variants", "current", "--output", str(root),
            ])
            ...
            def run_case(variant, task_dir, scenario, output, configured):
                ...
                calls.append(key)
                if key == ("low", "gpt-5.6-sol", 5, "current"):
                    raise RuntimeError("first arm failed")
                return {**result(variant, attempt(wall=1, cost=0.1)), "task_id": scenario["id"], "task_slug": scenario["slug"]}

            def submit(function, *values):
                model_indices.append(values[0])
                future = Mock()
            ...
            with patch.object(benchmark, "run_case", side_effect=run_case), \
                ...
                pool.return_value.__enter__.return_value.submit.side_effect = submit
                outputs = benchmark.run_campaign(args, scenarios, [19, 5], manifest)
                with self.assertRaisesRegex(ValueError, "--max-workers must be 1..6"):
                    benchmark.run_campaign(argparse.Namespace(**{**vars(args), "max_workers": 7}), scenarios, [19, 5], manifest)
            ...
            self.assertEqual(by_case[(5, "current")]["attempts"][0]["error"], "RuntimeError: first arm failed")
            self.assertEqual(by_case[(19, "current")]["attempts"][0]["judge"]["status"], "pass")
            self.assertEqual(len(calls), 4)
            self.assertEqual(model_indices, [0, 2, 0, 2])
            self.assertEqual(len(outputs), 2)
            self.assertEqual({(phase, variant) for phase, _model, _task, variant in calls}, {("low", "current")})
            self.assertIn(("low", "deepseek-flash", 19, "current"), calls)
            ...
            self.assertEqual(campaign["tasks"], [5, 19])
            self.assertEqual([model["label"] for model in campaign["models"]], ["sol", "deepseek"])
            self.assertEqual(campaign["model_outputs"], outputs)
            self.assertEqual([item["model"] for item in outputs], ["gpt-5.6-sol", "deepseek-flash"])
            for item in outputs:
                configured = json.loads((Path(item["output"]) / "invocation.json").read_text())
                self.assertEqual(configured["models"], campaign["models"])
                self.assertEqual(configured["model"], item["model"])
            self.assertFalse((root / "low/astra").exists())
            self.assertFalse((root / "medium").exists())


    def test_campaign_rejects_unknown_model_selector(self) -> None:
        with patch("sys.stderr", new_callable=io.StringIO) as stderr, self.assertRaises(SystemExit) as raised:
            benchmark.build_parser().parse_args(["--three-model-campaign", "--models", "sol,unknown"])
        self.assertEqual(raised.exception.code, 2)
        self.assertIn("unknown campaign models: ['unknown']", stderr.getvalue())
    ...
    def test_public_host_layout_and_selected_provider_launch(self) -> None:
        with tempfile.TemporaryDirectory(dir="/tmp") as directory:
            ...
            with patch.object(host_setup, "read_candidate", return_value=(
                ...
                current_output = root / "current-hosts"
                current_manifest = host_setup.prepare_hosts(None, candidate, dependencies, node, current_output, current_only=True)
            self.assertEqual(set(current_manifest["hosts"]), {"current"})
            self.assertFalse((current_output / "unpacked").exists())
            ...
            args = argparse.Namespace(hosts_manifest=output / "hosts.json", bwrap="/usr/bin/bwrap", variants="vanilla,current",
            ...
            args.host_openai_codex_auth_file.write_text("{}")
            args.host_deepseek_api_key_file.write_text("fixture-provider-secret")
            for credential in (args.host_openai_codex_auth_file, args.host_deepseek_api_key_file):
...
options = json.loads(Path(sys.argv[1]).read_text().rsplit("(host, ", 1)[1].removesuffix(");\n"))
relative = Path("workspace-contract.txt").read_text()
sdk = (Path(options["cwd"]) / "workspace-contract.txt").read_text()
Path("workspace-contract.txt").write_text("agent\n")
inner = subprocess.run(
    [sys.executable, "-E", "-S", "-B", sys.argv[2], "-c",
     "pwd; cat workspace-contract.txt; printf 'bash\\n' >> workspace-contract.txt"],
    check=True, capture_output=True, text=True, timeout=10,
)
print(json.dumps({"cwd": os.getcwd(), "bootstrap_cwd": options["cwd"],
                  "workspace_alias": Path("/workspace").is_dir(),
                  "relative": relative, "sdk": sdk, "inner": inner.stdout,
                  "final": Path("workspace-contract.txt").read_text()}))
"""
            for provider, model, variant, entry, mount, excluded in (
                ...
                roots = benchmark.prepare_agent_home(run_dir, args, variant)
                ...
                expected_cwd = "/workspace" if variant == "current" else str(workspace)
                self.assertEqual(isolated[isolated.index("--chdir") + 1], expected_cwd)
                self.assertEqual("/workspace" in isolated, variant == "current")
                (workspace / "workspace-contract.txt").write_text("fixture\n")
                (run_dir / "sandbox-runtime/services.json").write_text('{"services": []}')
                probe = benchmark.isolated_agent_command(
                    [benchmark.python312(), "-E", "-S", "-B", "-c", workspace_probe,
                     command[-1], str(roots["launcher"])], args.hosts[variant], run_dir, args,
                )
                completed = subprocess.run(
                    probe, capture_output=True, text=True, timeout=20, env={"PATH": "/usr/bin:/bin"},
                )
                self.assertEqual(completed.returncode, 0, completed.stdout + completed.stderr)
                self.assertEqual(json.loads(completed.stdout), {
                    "cwd": expected_cwd, "bootstrap_cwd": expected_cwd,
                    "workspace_alias": variant == "current", "relative": "fixture\n", "sdk": "fixture\n",
                    "inner": "/workspace\nagent\n", "final": "agent\nbash\n",
                })
            ...
            current_args = argparse.Namespace(hosts_manifest=current_output / "hosts.json", variants="current")
            self.assertEqual(set(benchmark.apply_hosts_manifest(current_args)["hosts"]), {"current"})
            with self.assertRaisesRegex(ValueError, "hosts manifest missing selected variants: .*vanilla"):
                benchmark.apply_hosts_manifest(argparse.Namespace(
                    hosts_manifest=current_output / "hosts.json", variants="vanilla,current"))
            # An unselected vanilla entry must not cause a package-tree read.
            current_manifest["hosts"]["vanilla"] = {"package_root": str(root / "unavailable-vanilla")}
            benchmark.json_dump(current_output / "hosts.json", current_manifest)
            self.assertEqual(benchmark.apply_hosts_manifest(current_args)["hosts"]["current"], current_manifest["hosts"]["current"])
...
"""
    ...
    def test_restore_and_clinic_module_and_package_entrypoint_parity(self) -> None:
        cases = (
            ("15-backup-restore-planner", "restore_plan", 1, "main outputs invalid:"),
            ("27-clinic-appointment-rescheduling", "clinic_schedule", 0, "malformed main output:"),
        )
        for task, module, passed, output_note in cases:
            judge = Path(__file__).parent / "tasks" / task / "judge.py"
            for layout in (f"{module}.py", f"{module}/__main__.py"):
                with self.subTest(task=task, layout=layout), tempfile.TemporaryDirectory() as directory:
                    candidate = Path(directory)
                    solution = candidate / "solution"
                    solution.mkdir()
                    (solution / "__init__.py").write_text("")
                    entrypoint = solution / layout
                    entrypoint.parent.mkdir(exist_ok=True)
                    # An empty executable must reach output validation, not fail the gate or launch.
                    entrypoint.write_text("pass\n")
                    completed = subprocess.run(
                        ["/usr/bin/python3.12", "-E", "-S", "-B", str(judge),
                    ...
                    self.assertEqual(result["main_checks_passed"], passed)
                    self.assertFalse(result["edge_check_passed"])
                    self.assertTrue(result["notes"][0].startswith(output_note), result["notes"])
                    self.assertFalse(any("command failed" in note for note in result["notes"]), result["notes"])
    ...
    def test_task17_markdown_escapes_preserve_comparison_requirements(self) -> None:
        spec = importlib.util.spec_from_file_location(
            "clause_judge", Path(__file__).parent / "tasks/17-contract-clause-index/judge.py")
        judge = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(judge)
        with tempfile.TemporaryDirectory() as directory:
            work = Path(directory)
            output = work / "output"
            output.mkdir()
            (output / "clauses.csv").write_text(",".join(judge.HEADER) + "\n", encoding="utf-8")
            (output / "missing.csv").write_text("contract_id,clause_type\n", encoding="utf-8")
            citations = "\n".join(f"C{number:03d}: lines 3–4" for number in range(1, 19))
            for fee_months, expected in ((6, True), (7, False)):
                with self.subTest(fee_months=fee_months):
                    comparison = (
                        "12 months; 24 months; does not automatically renew\n"
                        "California; New York; Texas\n"
                        f"usd:100000; fees\\_months:{fee_months}; usd:250000\n{citations}\n"
                    )
                    (output / "comparison.md").write_text(comparison, encoding="utf-8")
                    checks, parseable, _ = judge.check_main(work, completed=True)
                    self.assertTrue(parseable)
                    self.assertEqual(checks[3], expected)
