...
Fresh grading copies only your `crm/` package into a newly seeded workspace. It starts with a fresh copy of the supplied version-1 database, runs migration twice, then injects the import input and runs import twice against that same database. Your development database and reports are not copied. Preserve and update database records within each workflow; implement all required state changes in the package, not as one-off edits to your development database.
