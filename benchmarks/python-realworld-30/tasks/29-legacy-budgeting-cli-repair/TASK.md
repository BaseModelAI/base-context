...
Grading copies only `budgetdesk/` into fresh fixtures containing their own supplied
legacy database and all stage inputs. It runs two imports, a report, and both
exports against the same database. Development databases and outputs are not
copied. Preserve and migrate the supplied database; this task does not start with
an empty database. Category rules, budgets, and optional splits are found beside
the accounts file in its input directory.
