...
def utc_head(head: str, name: str) -> bool:
    parts = head.upper().split(";")
    return parts[0] == name and parts[1:] in ([], ["VALUE=DATE-TIME"])
...
def parse_calendar(path: Path) -> tuple[list[Event], bool]:
    ...
    for line in lines[1:-1]:
        ...
        if upper == "END:VEVENT":
            ...
            status = one(current, "STATUS", optional=True)
            if status and status[1].upper() == "CANCELLED":
                raise ParseError("cancelled output event")
            ...
            events.append(
                Event(
                    uid=text_value(uid[1]),
                    sequence=sequence_number,
                    start=start[1],
                    end=end[1],
                    summary=text_value(summary[1]),
                    location=None if location is None else text_value(location[1]),
                    start_head=start[0],
                    end_head=end[0],
                )
            )
...
def canonical_and_ordered(snapshot: Snapshot, expected: dict[str, Event]) -> bool:
    ...
    if len(snapshot.events) != len(expected) or not fields_match(actual, expected, ("start", "end")):
        return False
    if any(
        not utc_head(item.start_head, "DTSTART")
        or not utc_head(item.end_head, "DTEND")
