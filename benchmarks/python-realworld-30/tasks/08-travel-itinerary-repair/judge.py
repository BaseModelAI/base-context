...
def load_reference(inputs):
    ...
    for sid,update in highest.items():
        if sid in segments and int(update["revision"])>int(segments[sid]["revision"]): segments[sid].update(update)
    ...
    for sid in sorted(highest):
        ...
        for alt in alternatives:
            if alt["replaces_segment_id"]!=sid: continue
...
def read_rebook(path):
    ...
    if any(None in row or any(value is None for value in row.values()) for row in rows): raise ValueError("malformed rebook row")
    ...
    try:
        ...
        if artifact:
            ...
            if erun.returncode==0:
                ...
                except (OSError,UnicodeError,csv.Error,json.JSONDecodeError,ValueError,KeyError,TypeError,AttributeError): pass
