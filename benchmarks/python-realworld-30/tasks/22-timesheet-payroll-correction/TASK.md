...
not invent their time. Group by `(employee_id, shift_id)`. A valid group has exactly
one IN, one later OUT, and zero or more complete, nonoverlapping break pairs inside
them. Exclude an invalid group from paid time. Identify the offending punch record
in each exception and use a nonempty descriptive reason; no fixed reason vocabulary
is required.
...
- `payroll.csv`, header
  ...
  decimal places and money uses two decimal places, both with `ROUND_HALF_UP`.
  Compute employee totals from unrounded minute amounts, not rounded shift totals.
...
The final grading invocation receives all stage inputs together and starts with an
empty output directory. Reconstruct required before/after values from the supplied
input files; prior generated outputs are not available.
