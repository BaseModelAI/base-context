...
def outputs(root: Path):
    ...
    if not isinstance(summary, dict) or set(summary) != {"total_tickets", "by_priority"}:
        raise ValueError("summary shape")
    if not json_integer(summary["total_tickets"]) or not isinstance(summary["by_priority"], dict):
        raise ValueError("summary count types")
    for value in summary["by_priority"].values():
        if not isinstance(value, dict) or set(value) != {"tickets", "first_response_breaches", "resolution_breaches"} or any(not json_integer(count) for count in value.values()):
            raise ValueError("priority count types")
    ...
    try:
        if artifact:
            ...
            if run.returncode != 0:
                ...
                except (OSError, UnicodeError, csv.Error, json.JSONDecodeError, ValueError, KeyError, TypeError, AttributeError, StopIteration) as exc:
        ...
        if artifact:
            ...
            if edge_run.returncode == 0:
                ...
                except (OSError, UnicodeError, csv.Error, json.JSONDecodeError, ValueError, KeyError, TypeError, AttributeError, StopIteration):
                    pass
            else:
