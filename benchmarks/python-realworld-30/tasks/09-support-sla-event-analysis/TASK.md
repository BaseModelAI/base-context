...
Preserve the creation event's timestamp text in `created_at`. Minute counts
are base-10 whole-number strings without a decimal suffix. JSON counts are
integers, not booleans. Use lowercase `true`/`false`. Status is `resolved` or `open`. Sort rows by `ticket_id` in ascending Unicode order.
...
the input files, and runs the command. Existing output files are not copied.
