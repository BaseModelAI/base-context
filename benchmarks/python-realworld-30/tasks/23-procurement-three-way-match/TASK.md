...
Grading stages all receipt corrections and credit notes in a fresh workspace,
then runs the final command once. Development outputs are not copied. Apply
corrections before aggregation, then credits, using only those supplied inputs.
For repeated line credits, the cumulative amount applied to an invoice line
cannot exceed its original extension. `credit_applied` counts line credit
actually used. Supplier `header_credit` is the total header credit requested;
any part beyond the supplier subtotal is included in `unapplied_credit`.
JSON whitespace and standard CSV quoting styles are not significant. Required
money strings still have exactly two fractional digits.
