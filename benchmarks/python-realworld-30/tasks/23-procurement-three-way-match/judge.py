...
def csvrows(path):
 with path.open(newline="",encoding="utf-8") as f: result=list(csv.DictReader(f))
 if any(None in row or any(value is None for value in row.values()) for row in result):raise ValueError("malformed CSV row")
 return result
def header(path):
...
def expected(inp):
 ...
 credits=defaultdict(Decimal); excess=defaultdict(Decimal); header_credit=defaultdict(Decimal); used_line_credit=defaultdict(Decimal)
 if (inp/"credit_notes.xml").exists():
  for c in ET.parse(inp/"credit_notes.xml").getroot():
   ...
   if c.tag=="header_credit":header_credit[supplier]+=amount
    ...
    lid=c.attrib["invoice_line_id"]; key,owner,ext=line_nodes[lid]; applied=min(amount,max(Decimal(0),ext-used_line_credit[lid]));used_line_credit[lid]+=applied;credits[key]+=applied;excess[supplier]+=amount-applied
 ...
 for r in lines:
  s=r["supplier_id"];sumv[s]["line_gross"]+=Decimal(money(gross[r["po_id"],r["line_id"]]));sumv[s]["credit_applied"]+=Decimal(r["credit_applied"]);sumv[s]["line_payable"]+=Decimal(r["line_payable"])
 summaries=[]
...
def parse_outputs(out):
 got=csvrows(out/"line_matches.csv"); exc=csvrows(out/"exceptions.csv"); summary=json.loads((out/"supplier_summary.json").read_text())
 if header(out/"line_matches.csv")!=LINE_HEAD or header(out/"exceptions.csv")!=EX_HEAD:raise ValueError("CSV header")
 if not isinstance(summary,dict) or set(summary)!={"suppliers"} or not isinstance(summary["suppliers"],list):raise ValueError("supplier summary shape")
 return got,exc,summary
...
def maincheck(candidate):
 ...
 try:
  ...
  try:
   got,exc,summary=parse_outputs(out);parsed=True
  ...
  qfields=["supplier_id","item_id","ordered_qty","received_qty","invoiced_qty","quantity_variance"]
  check1=parsed and len(got)==len(gm)==len(exp) and list(gm)==list(em) and all(all(gm.get(k,{}).get(f)==em[k][f] for f in qfields) for k in em)
...
def edgecheck(candidate):
 ...
 try:
  ...
  try:
   got,exc,summary=parse_outputs(holder/"workspace/output"); exp,eexc,esummary=expected(holder/"workspace/inputs")
   return got==exp and exc==eexc and summary==esummary,diag
