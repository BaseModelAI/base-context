...
6. A purchase arrival is `as-of + lead_days`. Later supplier blackouts include
   both their start and end dates; advance an affected arrival until it lies
   outside every blackout for that supplier.

Use decimal arithmetic for means and targets. Format forecast and target values with four digits after the decimal point
using round-half-even; do not use binary floating point for those output values. For whole-unit transfer and purchase decisions, compute target stock and donor safety floor directly as the 28-day demand total multiplied by the applicable days and divided by 28, so an exact-integer result stays exact; do not multiply a rounded repeating daily mean.
...
all inputs including the later constraints, and runs the final command once.
Existing output files are not copied; compute transfers and purchases from
the supplied inputs rather than a previous generated plan.
