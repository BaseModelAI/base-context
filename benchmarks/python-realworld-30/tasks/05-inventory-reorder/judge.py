...
FOUR_DECIMAL = re.compile(r"^-?[0-9]+\.[0-9]{4}$")
...
def reference(inputs: Path) -> dict[str, object]:
    ...
    if transfer_path.exists():
        ...
        for row in cost_rows:
            ...
            cost_text[(row["from_warehouse"], row["to_warehouse"])] = row["cost_per_unit"]
        ...
        for recipient in recipients:
            ...
            while need:
                ...
                transfers.append({
                    ...
                    "quantity": quantity, "cost_per_unit": cost_text[(donor, recipient)],
                })
...
def parse_transfers(path: Path) -> list[dict[str, object]]:
    ...
    if fields != ["sku", "from_warehouse", "to_warehouse", "quantity", "cost_per_unit"]:
        raise ValueError("wrong transfers.csv header")
    result = []
    for row in rows:
        if not WHOLE_NUMBER.fullmatch(row["quantity"]) or int(row["quantity"]) <= 0:
            raise ValueError("transfer quantity must be a positive integer")
        result.append({**row, "quantity": int(row["quantity"])})
    if result != sorted(result, key=lambda row: (row["sku"], row["to_warehouse"], row["from_warehouse"])):
        raise ValueError("transfer row order")
    return result
...
def parse_outputs(workspace: Path) -> tuple[list[dict[str, str]], dict[str, object], list[dict[str, object]]]:
    ...
    if set(supplier_doc) != {"as_of", "suppliers"}:
        raise ValueError("supplier document keys")
    for supplier in supplier_doc["suppliers"]:
        if not isinstance(supplier, dict) or not isinstance(supplier.get("lines"), list):
            raise ValueError("supplier shape")
        if any(not json_integer(supplier.get(field)) for field in ("minimum_cases", "total_cases")):
            raise ValueError("supplier counts must be integers")
        for line in supplier["lines"]:
            if not isinstance(line, dict) or any(not json_integer(line.get(field)) for field in ("quantity", "cases")):
            ...
            try:
                ...
                if purchase_ok:
                    for key in keys:
                        ...
                        purchase_ok &= (int(row["case_pack"]) > 0 and int(row["reorder_qty"]) % int(row["case_pack"]) == 0)
                ...
                normalize = lambda items: sorted(
                    (x["sku"], x["from_warehouse"], x["to_warehouse"], int(x["quantity"]), x["cost_per_unit"])
                    for x in items
                )
