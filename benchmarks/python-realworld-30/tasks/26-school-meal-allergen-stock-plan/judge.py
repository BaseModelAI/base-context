...
def edge_purchase_matches(purchases):
    expected = {}
    def text(value):
        result=format(value,"f")
        return result.rstrip("0").rstrip(".") if "." in result else result
    for ingredient,required in EDGE_REQUIRED.items():
        stock=Decimal(MAIN_PURCHASE[ingredient][1]); pack=Decimal(MAIN_PURCHASE[ingredient][3])
        shortfall=max(required-stock,Decimal(0)); packs=(shortfall/pack).to_integral_value(rounding=ROUND_CEILING)
        expected[ingredient]=(text(required),text(stock),text(shortfall),text(pack),str(int(packs)),text(packs*pack))
    actual={row["ingredient_id"]:tuple(row[field] for field in PURCHASE_COLUMNS[1:]) for row in purchases}
    return actual==expected and len(actual)==len(purchases)
...
def edge_check(workspace: Path) -> tuple[bool, str]:
    try:
        ...
        passed = (
            ...
            and edge_purchase_matches(purchases)
        )
