...
Grading copies all of `solution/` into fresh main and edge workspaces, adds
the substitution input, and runs the final command once. Development output
files are not copied. Compute both original and substituted recipe results
from these inputs, not a previous eligibility report. The main and edge
substitution inputs differ, so do not retain a graph from another run.
Standard CSV quoting styles are equivalent. All five report headers are
required even for empty reports.
