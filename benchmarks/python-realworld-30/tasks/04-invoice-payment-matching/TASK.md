...
Every monetary field in every output uses exactly two fractional digits and no
leading plus sign. Leading zeroes and a signed zero are equivalent amounts. CSV files are UTF-8 and are written with `newline=""`.
...
the inputs, and runs the command. Existing output files are not copied.
