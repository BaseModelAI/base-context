...
def check_main(work: Path, completed: bool) -> tuple[list[bool], bool, list[str]]:
    ...
    try:
        ...
        # Markdown backslash escapes display their literal punctuation characters.
        comparison = re.sub(r"\\([" + re.escape(string.punctuation) + "])", r"\1", comparison)
        keys = [(row["contract_id"], row["clause_type"]) for row in rows]
        if keys != sorted(set(keys)):
            raise ValueError("clauses must be unique and in lexical order")
        for row in rows:
            if not 1 <= int(row["start_line"]) <= int(row["end_line"]):
                raise ValueError("invalid clause line range")
        parseable = True
    ...
    groups_present = (
        ...
        # Markdown may use formatted amounts or normalized CSV values.
        and re.search(r"(?<![\d,])100,?000(?![\d,])", comparison_folded) is not None
        and ("6 months" in comparison_folded or "fees_months:6" in comparison_folded)
        and re.search(r"(?<![\d,])250,?000(?![\d,])", comparison_folded) is not None
    )
