...
Fresh grading copies only your `solution/` directory. It regenerates the initial contracts and supplied review-set and late-contract inputs, then runs the command once on the final inputs. Your generated CSV and Markdown reports and scratch files are not copied. The command must recreate every requested output.
