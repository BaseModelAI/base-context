...
Fresh grading copies only your `solution/` directory. It regenerates the knowledge base, redirects, and supplied rename inputs, then runs the command on that final input tree. Your generated output and scratch files are not copied. Recreate the repaired tree and both reports from those inputs.
