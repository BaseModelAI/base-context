...
- `output/baseline_issues.csv`, exact header
  ...
  order, breaking further ties by detail. The possible codes are `BAD_SLOT`, `PATIENT_WINDOW`,
  `PROVIDER_AVAILABILITY`, `ROOM_CAPABILITY`, `PROVIDER_OVERLAP`,
  `ROOM_OVERLAP`, and `TRAVEL_BUFFER`. Use the patient ID for `PATIENT_WINDOW`,
  provider ID for `PROVIDER_AVAILABILITY`, and room ID for `ROOM_CAPABILITY`.
  For either overlap code, report both appointments with the other appointment
  ID as detail. Report `TRAVEL_BUFFER` on the later appointment with its
  preceding appointment ID as detail. For `BAD_SLOT`, use a nonempty description
  of the invalid start or required grid; no fixed prose is required.
...
Grading copies only `solution/` into independent fresh workspaces for the initial
inputs and for all final-stage inputs together, plus an edge fixture. Generated
outputs from development or another grading invocation are not copied. Derive
baseline reports and changes from the supplied inputs on each invocation.
