...
Displayed times and durations use round-half-up to three decimal places. Do not
round transcript times before clipping or summing. JSON member names must be
unique; chapter numbers are numeric integers (`1` and `1.0` are equivalent), not booleans.
...
Grading copies only `solution/audio_chapters.py` and `solution/__init__.py` into
fresh main and edge workspaces. No generated output is retained. It invokes the
final command independently for each input, with all later-stage inputs present.
