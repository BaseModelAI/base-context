...
def read_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(chapters, list) or any(
        not isinstance(chapter, dict) or not integer_number(chapter.get("number"))
        for chapter in chapters
    ):
        raise ValueError("chapters must contain objects with integer numbers")
    return value
...
def main() -> None:
    ...
    try:
        ...
        if not main_statuses or not all(main_statuses.values()):
            checks = [False] * 5
