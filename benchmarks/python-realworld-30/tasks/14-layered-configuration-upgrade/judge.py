...
def expected_report(config: dict[str, Any] = EXPECTED_CONFIG) -> str:
    lines = ["Layered Configuration Upgrade Report", "[effective]"]
    for path, value in sorted(leaf_items(config)):
    ...
    for path, _ in sorted(leaf_items(config)):
...
def check_main(config: dict[str, Any], report: str) -> list[bool]:
    ...
    report_correct = (
        same_json_type(config, EXPECTED_CONFIG)
        and report == expected_report(config)
