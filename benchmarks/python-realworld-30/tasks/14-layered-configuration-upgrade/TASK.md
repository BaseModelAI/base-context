...
Fresh grading copies only your `solution/` directory into a newly seeded workspace and runs the command on the supplied configuration files. It also runs a separate substitution-cycle fixture. Your generated output and scratch files are not copied; the command must recreate its outputs from its input directory.
