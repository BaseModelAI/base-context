...
def parse_timeline(path: Path) -> list[dict[str, str]]:
    ...
    for row in rows:
        ...
        if int(row["source_line"]) < 1:
            raise ValueError("source_line must be positive")
    keys = [(row["timestamp_utc"], row["service"], row["source_file"], int(row["source_line"])) for row in rows]
    if keys != sorted(keys):
        raise ValueError("timeline rows are not in the required order")
    return rows
...
def evaluate_main(candidate: Path, workspace: Path) -> tuple[list[bool], bool, bool, list[str]]:
    ...
    try:
        ...
        checks[3] = (
            ...
            and "customer_status" in report.casefold()
            and re.search(r"\b(?:drop(?:ped|ping)?|remov(?:e[ds]?|ing|al))\b", report, re.IGNORECASE) is not None
        )
