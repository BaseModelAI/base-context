...
Fresh grading copies only your `solution/` and `monitor/` directories. It regenerates the log inputs and any supplied follow-up inputs, runs the batch command with all final-stage inputs, and invokes `monitor/monitor.py` with a separate standard-input stream. Your generated output files and scratch files are not copied. The programs must recreate their required outputs from the supplied inputs.
...
claiming an unsupported clock correction. Name the implicated release and schema
object, and explain the schema operation in ordinary prose; no exact wording is required. Include exactly three primary source
