...
def rows(p):
 with p.open(newline="",encoding="utf8") as f: result=list(csv.DictReader(f))
...
def fresh(candidate,fixture):
 ...
 if code:return h,code,d
 copy(p,w)
 notices=w/"output/notices";notices.mkdir(parents=True,exist_ok=True);(notices/"stale-notice.txt").write_text("stale notice\n")
...
def output_rows(out):
 result=[]
 for filename,fields in (("active_loans.csv",ACTIVE),("fines.csv",FINES),("correction_effects.csv",EFFECT)):
  if head(out/filename)!=fields:raise ValueError("output CSV header")
  result.append(rows(out/filename))
 return result

def notices_ok(out,patrons,asof,by,fines):
 folder=out/"notices"; names=sorted(p.name for p in folder.glob("*.txt")); expected_names=[pid+".txt" for pid in sorted(patrons)]; fine_map={r["patron_id"]:r for r in fines}
 return names==expected_names and all((folder/(pid+".txt")).read_bytes()==notice(patrons[pid],asof,by[pid],fine_map[pid]).encode("utf8") for pid in patrons)
...
def maincheck(candidate):
 ...
 try:
  ...
  try:
   ga,gf,ge=output_rows(out);parsed=True
  ...
  check4=notices_ok(out,patrons,asof,eby,ef)
...
def edgecheck(candidate):
 ...
 try:
  ...
  try:
   out=h/"workspace/output";a,f,e=output_rows(out);asof="2025-08-31T23:59:59Z"
   patrons,ea,ef,by=reconstruct(h/"workspace/inputs",asof,True)
   return a==ea and f==ef and e==effects(h/"workspace/inputs",asof) and notices_ok(out,patrons,asof,by,ef),d
