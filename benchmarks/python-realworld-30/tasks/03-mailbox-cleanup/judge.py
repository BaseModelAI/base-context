...
def mime_fact(message):
    headers = tuple((name.lower(), decode_value(value)) for name, value in message.items()
                    if name.lower() not in {"content-type", "content-transfer-encoding"})
    parameters = tuple(sorted((name.lower(), value) for name, value in message.get_params()[1:]
                              if name.lower() != "boundary")) if message.get_params() else ()
    payload = tuple(mime_fact(part) for part in message.get_payload()) if message.is_multipart() else message.get_payload(decode=True)
    return headers, message.get_content_type(), parameters, payload
...
def orphan_names(threads: list, cleaned: list, retained: list) -> dict[str, str]:
    """Resolve arbitrary orphan IDs from the matching mbox and CSV order."""
    def orphan(message):
        return not message_id(message) and not message_ids(message.get("References")) and not message_ids(message.get("In-Reply-To"))
    remaining = [item for item in retained if orphan(item[1])]
    generated = {thread_id(message, ordinal) for ordinal, message in remaining}
    expected_rows = {row["thread_id"]: row for row in expected_threads(retained)}
    known = set(expected_rows) - generated
    if len({row["thread_id"] for row in threads}) != len(threads):
        raise ValueError("duplicate thread ID")
    unknown_rows = [row for row in threads if row["thread_id"] not in known]
    messages = [message for message in cleaned if orphan(message)]
    if len(messages) != len(remaining) or len(unknown_rows) != len(remaining):
        raise ValueError("orphan count")
    names = {}
    for row, message in zip(unknown_rows, messages):
        match = next((item for item in remaining if message_fact(item[1]) == message_fact(message)), None)
        if match is None or not row["thread_id"]:
            raise ValueError("orphan content")
        root = thread_id(match[1], match[0])
        if any(row[field] != expected_rows[root][field] for field in THREAD_FIELDS[1:]):
            raise ValueError("orphan thread facts")
        names[root] = row["thread_id"]
        remaining.remove(match)
    return names


def cleaned_order_ok(messages: list, retained: list[tuple[int, object]], names: dict[str, str]) -> bool:
    expected = sorted(retained, key=lambda item: (
        names.get(thread_id(item[1], item[0]), thread_id(item[1], item[0])),
        parsed_date(item[1]) is None,
        parsed_date(item[1]) or datetime.max.replace(tzinfo=timezone.utc),
        item[0],
    ))
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            try:
                ...
                names = orphan_names(threads, cleaned, retained)
                expected_thread_rows = expected_threads(retained)
                for row in expected_thread_rows:
                    row["thread_id"] = names.get(row["thread_id"], row["thread_id"])
                expected_thread_rows.sort(key=lambda row: row["thread_id"])
                ...
                main_checks[4] = (
                    ...
                    and cleaned_order_ok(cleaned, retained, names)
                )
