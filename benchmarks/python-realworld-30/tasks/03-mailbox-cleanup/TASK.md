...
- Determine a message's thread root from the first message ID in `References`; if absent, use the first ID in `In-Reply-To`; if absent, use its own `Message-ID`. A no-ID orphan may use any stable, unique generated thread ID that does not
  collide with another thread ID. Use that same ID to order its mbox entry and
  its CSV row.
...
1. `cleaned.mbox`, containing the retained original messages, including original subject prefixes,
   address-header roles, MIME parts and attachments. Equivalent mbox/MIME
   serialization is allowed; do not change the message content. Sort by `thread_id`; within a thread put valid dates in chronological order, then unknown dates; break remaining ties by retained input order.
2. `threads.csv`, with this exact header:
   ...
   Write one row per thread, sorted by `thread_id`. `subject` comes from the first retained message in the cleaned chronological
   order above with a non-empty display subject. Participants are the sorted union for the thread. `first_date` and `last_date` are the earliest and latest valid dates; leave both blank when a thread has no valid date. Count retained messages, including no-ID messages.
...
Grading copies only `solution/` into fresh workspaces. The main command
receives both mbox paths in command-line order; the edge command receives
only the current mbox. Existing output files are not copied.
