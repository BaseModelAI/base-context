...
Fresh grading copies only your `solution/` directory into a newly seeded workspace. It runs `build` and `search`, then supplies the update inputs and runs `update` and further searches against that same database. Your development database and duplicate report are not copied. Keep the database self-contained and preserve its state between commands within the workflow; only `build` replaces it.
