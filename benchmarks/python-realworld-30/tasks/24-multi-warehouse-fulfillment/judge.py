...
import argparse,csv,itertools,json,re,shutil,subprocess,tempfile
...
def output_rows(out):
 reports=[]
 for name,fields,numbers in (("allocations.csv",ALLOC,["quantity"]),("backorders.csv",BACK,["requested_qty","allocated_qty","backorder_qty"]),("inventory_after.csv",INV,["on_hand","allocated_qty","remaining_qty"]),("allocation_changes.csv",CHANGE,["before_qty","after_qty","delta_qty"])):
  path=out/name; raw=path.read_bytes()
  if b"\r" in raw or not raw.endswith(b"\n") or head(path)!=fields:raise ValueError("output header or line endings")
  report=rows(path)
  for row in report:
   for field in numbers:
    if not re.fullmatch(r"-?[0-9]+",row[field]):raise ValueError("non-integer output quantity")
    row[field]=str(int(row[field]))
    if field!="delta_qty" and int(row[field])<0:raise ValueError("negative output quantity")
   if "status" in row and row["status"] not in {"shipped","reserved"}:raise ValueError("allocation status")
  reports.append(report)
 if any(int(row["quantity"])<=0 for row in reports[0]) or any(int(row["backorder_qty"])<=0 for row in reports[1]):raise ValueError("nonpositive output row")
 return reports
...
def maincheck(candidate):
 ...
 try:
  ...
  try:
   ga,gb,gi,gc=output_rows(out);parsed=True
...
def edgecheck(candidate):
 ...
 try:
  ...
  try:
   a,b,i,c=output_rows(h/"workspace/output"); final,changes,_,_=expected(h/"workspace/inputs")
   return (a,b,i)==final and c==changes,d
