...
Grading stages both later input files in a fresh workspace and then runs the
final command once. Development outputs and intermediate stage runs are not
copied or replayed. When later inputs are present, the implementation must
reconstruct the initial plan, the outage plan, and then the expedited plan
from the supplied inputs. `existing_allocations.csv` is source input, not a
complete initial generated plan. Keep the required allocation changes between
those computed plans; a correct final allocation alone is not sufficient.
When later inputs exist, emit `allocation_changes.csv` even if it has only
its declared header. A repeated run with the same inputs must regenerate the
same final outputs and change history rather than treating a previous final
output as the original baseline. Standard CSV quoting styles are equivalent.
