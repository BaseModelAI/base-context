...
Emit one row per retained SKU, sorted by `sku` in ascending Unicode code-point order. Use normal Python CSV quoting rules; quoting every field or only fields that
need quoting are equivalent. Do not add any other columns, comments, timestamps, or nondeterministic content.
...
Grading copies only `solution/` into fresh main and edge workspaces and starts
a new loopback fixture server for each run. It supplies the current server
address in `inputs/base_url.txt`; do not retain an earlier port or catalog.
Existing output files are not copied. The supplied server, unlike a process
you start in a tool call, is managed by the runner for the task.
