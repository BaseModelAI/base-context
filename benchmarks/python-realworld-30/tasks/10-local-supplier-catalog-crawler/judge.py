...
def csv_matches(result):
    raw=result["raw"]
    expected=[dict(zip(HEADER,row)) for row in result["expected"]]
    return b"\r" not in raw and raw.endswith(b"\n") and result["records"]==expected
