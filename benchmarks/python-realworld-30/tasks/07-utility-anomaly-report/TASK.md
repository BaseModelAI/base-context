...
Format all numeric fields with exactly six digits after the decimal point,
using round-half-even. Sort using these emitted numeric severity values. Sort anomalies by descending numeric severity, then `meter_id`, then `month` in ascending Unicode order.
...
Create a self-contained UTF-8 `output/report.html` with no JavaScript or external resources. It must state the exact total meter, anomaly, and gap counts and contain a table of the ten largest anomalies in the same order as the CSV (or all rows when fewer than ten). Each table row must identify its meter and month; other table columns and
HTML layout are flexible. Associate each count with its visible label
(`meters`, `anomalies`, or `gaps`), for example `Total meters: 12`.
Escape all dynamic HTML text, including meter IDs. Named and numeric HTML
character references are equivalent. CSV files and the HTML file must use LF line endings.
...
the input CSV, and runs the command. Existing output files are not copied.
