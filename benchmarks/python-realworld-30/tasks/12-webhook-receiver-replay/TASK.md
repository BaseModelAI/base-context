...
Fresh grading copies only your `webhook_app/` package into a newly seeded workspace. It creates a new delivery database, starts loopback services, and runs receiver, worker, restart, replay, and idempotency checks against that same database. Your development database, logs, and outputs are not copied. Delivery state must persist in the named SQLite file across the commands and service restarts within a workflow. The judge owns service processes; do not depend on a server you leave running from an earlier tool call.
