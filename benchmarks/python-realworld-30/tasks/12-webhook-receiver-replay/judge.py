...
def accepted_id(status: int, value: Any) -> int | None:
    ...
    if type(event_id) in (int, float) and event_id > 0 and event_id % 1 == 0:
        return int(event_id)
    return None
...
def state_matches(
    ...
    *,
    ...
    return (
        code == 200
        and type(value.get("id")) in (int, float)
        and value.get("id") == event_id
        and value.get("status") == status
        and type(value.get("attempt_count")) in (int, float)
...
def database_row_counts(path: Path) -> dict[str, int]:
...
def evaluate_edge(candidate_workspace: Path, destination: Path) -> tuple[bool, list[str]]:
    ...
    try:
        ...
        receiver, base_url = start_receiver(destination)
        before = database_row_counts(destination / "workspace" / "webhooks.db")
        ...
        after = database_row_counts(destination / "workspace" / "webhooks.db")
        return status == 400 and missing_status == 404 and after == before, notes
    except Exception as exc:
