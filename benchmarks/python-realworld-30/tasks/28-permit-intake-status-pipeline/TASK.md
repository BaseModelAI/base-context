...
validation. A parse error makes the status `manual_review`. Continue independent
checks, but do not invent issues or values for checks that require an invalid
field. The supplied source files themselves are valid JSON and XML.
...
Fresh grading copies only your `permitflow/` package into a newly seeded
workspace. It supplies all ordinance, correction-batch, and final-status inputs,
then runs two imports and one validation against the same fresh database. Your
development database, reports, and scratch files are not copied. Implement all
state changes in the package; preserve records, internal IDs, and attachment
associations across commands within the workflow. Do not depend on one-off edits
to a development database or reports saved during an earlier stage.
