...
Fresh grading copies only your `solution/` directory. It regenerates the readings and supplied correction and explanation-format inputs, then runs the command on the final inputs. Your development bills and reports are not copied. Compute both original and corrected totals from the supplied data and recreate every requested output; do not depend on bills saved by an earlier stage.
