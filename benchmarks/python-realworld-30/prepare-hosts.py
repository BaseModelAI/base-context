...
def prepare_hosts(
    ...
    *, public_artifacts: Path | None = None, current_only: bool = False,
) -> dict[str, Any]:
    """Use local artifacts only. The sole executed command is Node --version."""
    if current_only:
        if h_artifacts is not None or public_artifacts is not None:
            raise ValueError("current-only preparation does not accept vanilla artifacts")
    elif (h_artifacts is None) == (public_artifacts is None):
        raise ValueError("provide exactly one of h_artifacts or public_artifacts")
    public = public_artifacts is not None
    artifact_root = None if current_only else Path(public_artifacts if public else h_artifacts).expanduser().resolve(strict=True)
    ...
    for source in (candidate_root, dependency_root, *([artifact_root] if artifact_root is not None else [])):
        if root.is_relative_to(source):
            raise ValueError(f"host root must be outside input directories: {source}")
    stock_packages = {} if current_only else (read_public_artifacts(artifact_root) if public else read_h_artifacts(artifact_root))
...
def main() -> int:
    ...
    artifacts.add_argument("--current-only", action="store_true", help="prepare only the native candidate without reading vanilla artifacts")
    ...
    manifest = prepare_hosts(
        ...
        public_artifacts=args.public_artifacts, current_only=args.current_only,
    )
