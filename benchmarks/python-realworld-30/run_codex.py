...
# Keep the original Sol publication rates and defaults unchanged.
...
def verify_clean_context(workspace: Path, codex_home: Path, *, initial: bool = True) -> None:
    ...
    if workspace.exists():
        found.extend(str(path) for name in ("AGENTS.md", "AGENTS.override.md") for path in workspace.rglob(name))
    if found:
...
def initial_command(codex: str, workspace: Path, last_message_path: Path, model: str = MODEL, thinking: str = REASONING_EFFORT) -> list[str]:
    return [
        codex,
        "exec",
        ...
        "--skip-git-repo-check",
        "--model", model,
        "--config", f'model_reasoning_effort="{thinking}"',
        "--json",
        "--color", "never",
...
def resume_command(codex: str, workspace: Path, thread_id: str, last_message_path: Path, model: str = MODEL, thinking: str = REASONING_EFFORT) -> list[str]:
    return [
        codex,
        "exec",
        "--sandbox", "workspace-write",
...
def equivalent_cost(usage: dict[str, int], rates: dict[str, float]) -> dict[str, float]:
    ...
    cache_write_tokens = int(usage.get("cache_write_input_tokens", 0)) if "cache_write_input" in rates else 0
    uncached_tokens = max(0, input_tokens - cached_tokens - cache_write_tokens)
    uncached_cost = uncached_tokens * rates["input"] / 1_000_000
    cache_write_cost = cache_write_tokens * rates.get("cache_write_input", 0.0) / 1_000_000
    ...
    return {
        ...
        **({"cache_write_input": cache_write_cost} if "cache_write_input" in rates else {}),
    }


def astra_session_cost(codex_home: Path, thread_id: str | None, usage: dict[str, int]) -> tuple[dict[str, float | None], str]:
    """Price request records, never the cumulative exec turn total, at the long-context tier."""
    ...
    if thread_id:
        for path in (codex_home / "sessions").rglob(f"*{thread_id}.jsonl"):
            ...
            for line in path.read_text().splitlines():
                ...
                if payload.get("type") != "token_count" or not info.get("last_token_usage"):
                    continue
                total = info.get("total_token_usage")
                # Rate-limit updates can repeat the previous token_count event.
                if total == previous_total:
                    continue
                previous_total = total
                requests.append(info["last_token_usage"])
    request_totals = sum_usage([{"usage": request} for request in requests])
    if not requests or any(request_totals.get(key, 0) != usage.get(key, 0)
                           for key in ("input_tokens", "cached_input_tokens", "output_tokens")):
        # JSONL turn totals alone cannot establish individual request context sizes.
        return {"total": None}, "unavailable: request usage absent or unreconciled; raw usage preserved"
    costs = {key: 0.0 for key in ("uncached_input", "cached_input", "cache_write_input", "output", "total")}
    for request in requests:
        long_context = request.get("input_tokens", 0) > ASTRA_LONG_CONTEXT_THRESHOLD
        rates = {key: rate * ((1.5 if key == "output" else 2.0) if long_context else 1.0)
                 for key, rate in ASTRA_RATES.items()}
        for key, value in equivalent_cost(request, rates).items():
            costs[key] += value
    return costs, "cost using matched Astra Standard per-request short/long-context rates"
    ...
    *,
    ...
    try:
        for index, stage in enumerate(scenario["stages"]):
            ...
            verify_clean_context(workspace, codex_home, initial=False)
            command = initial_command(codex, workspace, last_message_path, model, thinking) if index == 0 else resume_command(codex, workspace, thread_id or "", last_message_path, model, thinking)
    ...
    cost_kind = "cost using matched rates"
    if pricing == "astra-standard":
        costs, cost_kind = astra_session_cost(codex_home, thread_id, usage)
    input_tokens = int(usage.get("input_tokens", 0))
    cached_tokens = int(usage.get("cached_input_tokens", 0))
    ...
    return {
        ...
        "metrics": {
            ...
            "api_equivalent_cost_kind": cost_kind,
...
def run_attempt(
    ...
    try:
        ...
        result = {
            ...
            "metrics": {"subscription_billed_cost": None, "api_equivalent_cost": {"total": None}, "api_equivalent_cost_kind": "unavailable: runner failure", "model_api_calls": None, "model_api_calls_note": "Codex exec JSONL does not expose underlying model request count.", "codex_turns": 0, "provider_usage": {"input": 0, "cachedInput": 0, "cacheWriteInput": 0, "uncachedInput": 0, "output": 0, "reasoningOutput": 0, "totalTokens": 0}, "raw_usage": {}},
            "error": f"{type(exc).__name__}: {exc}",
        }
        json_dump(attempt_dir / "result.json", result)
        return result
...
def attempt_key(item: dict[str, Any]) -> tuple[int, int, int, int, float, float]:
    ...
    cost = ((item.get("metrics") or {}).get("api_equivalent_cost") or {}).get("total")
    equivalent = float(cost) if cost is not None else float("inf")
...
def build_summary(tasks: list[dict[str, Any]], invocation: dict[str, Any]) -> dict[str, Any]:
    ...
    def aggregate(items: list[dict[str, Any]]) -> dict[str, Any]:
        return {
            ...
            "subscription_billed_cost": None,
            "api_equivalent_cost": (sum(item["metrics"]["api_equivalent_cost"]["total"] for item in items)
                                    if all(item["metrics"]["api_equivalent_cost"]["total"] is not None for item in items) else None),
            "api_equivalent_cost_kinds": sorted({item["metrics"]["api_equivalent_cost_kind"] for item in items}),
            "model_api_calls": None,
...
def display_cost(value: float | None) -> str:
...
def write_summary_markdown(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Cost: **{display_cost(selected['api_equivalent_cost'])}**",
        f"- Cost accounting: {'; '.join(selected['api_equivalent_cost_kinds'])}",
    ...
    for item in summary["selected_tasks"]:
        ...
        lines.append(
            f"| {item['task_id']} | {result} | {item['agent_wall_seconds']:.3f} | {display_cost(item['api_equivalent_cost'])} | {item['codex_turns']} | {item['provider_usage']['totalTokens']:,} | A{item['selected_attempt']} |"
        )
    path.write_text("\n".join(lines) + "\n")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--thinking", "--reasoning-effort", default=REASONING_EFFORT)
    parser.add_argument("--expected-codex-version", help="Require an exact CLI version, for example 0.153.3")
    parser.add_argument("--pricing", choices=("historical", "astra-standard"), default="historical",
                        help="Historical custom flat rates, or Astra Standard per-request short/long-context rates")
...
def main() -> int:
    ...
    if args.expected_codex_version and version_text != f"codex-cli {args.expected_codex_version}":
        raise RuntimeError(f"Expected Codex {args.expected_codex_version}, got {version_text}")
    scenarios = load_scenarios(ROOT)
    task_ids = parse_task_ids(args.tasks, scenarios)
    publication_protocol = (
        task_ids == list(range(1, 31)) and args.max_workers == 6 and args.retry_failed == 1
        and args.model == MODEL and args.thinking == REASONING_EFFORT and args.pricing == "historical"
    )
    rates = {"input": args.input_rate, "cached_input": args.cached_input_rate, "output": args.output_rate}
    if args.pricing == "astra-standard":
        if args.model != "gpt-6-astra" or rates != DEFAULT_RATES:
            raise ValueError("astra-standard requires --model gpt-6-astra and no custom rate overrides")
        rates = dict(ASTRA_RATES)
    ...
    invocation = {
        ...
        "api_equivalent_pricing": args.pricing,
        "api_equivalent_pricing_sources": PRICING_SOURCES if args.pricing == "astra-standard" else [],
        "api_equivalent_long_context": ({"input_tokens_above": ASTRA_LONG_CONTEXT_THRESHOLD,
                                         "input_and_cache_multiplier": 2.0, "output_multiplier": 1.5,
                                         "scope": "full individual request, not cumulative turn/session usage"}
                                        if args.pricing == "astra-standard" else None),
    ...
    def submit_attempt(task_id: int, attempt: int) -> dict[str, Any]:
        ...
        result = run_attempt(codex, auth_file, codex_home, task_dir, scenario, output, attempt, args.timeout_seconds, args.bwrap, rates, args.model, args.thinking, args.pricing)
        ...
        print(
            ...
            f"{usage['totalTokens']} tokens {display_cost(result['metrics']['api_equivalent_cost']['total'])} cost",
            flush=True,
        )
        return result
