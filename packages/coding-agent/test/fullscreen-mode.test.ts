...
describe("fullscreen mode settings", () => {
	...
	beforeEach(() => {
		...
		savedEnv = process.env.BASE_CONTEXT_FULLSCREEN;
		savedTermProgram = process.env.TERM_PROGRAM;
		delete process.env.BASE_CONTEXT_FULLSCREEN;
	...
	afterEach(() => {
		...
		if (savedEnv === undefined) {
			...
			process.env.BASE_CONTEXT_FULLSCREEN = savedEnv;
		}
	...
	it("BASE_CONTEXT_FULLSCREEN env var overrides the setting in both directions", () => {
		const manager = SettingsManager.create(projectDir, agentDir);
		manager.setFullscreen(false);

		process.env.BASE_CONTEXT_FULLSCREEN = "1";
		expect(manager.getFullscreen()).toBe(true);

		manager.setFullscreen(true);
		process.env.BASE_CONTEXT_FULLSCREEN = "0";
