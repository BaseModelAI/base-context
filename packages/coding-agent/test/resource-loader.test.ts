...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".base-context", "prompts");
			...
			const projectSkillDir = join(cwd, ".base-context", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".base-context", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".base-context", "themes"), { recursive: true });
	...
	pi.registerCommand("shared", {
		...
		handler: async () => {},
			...
			symlinkSync(sharedExtDir, join(cwd, ".base-context", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".base-context", "extensions", "shared.ts"));
		});

		it("should keep both extensions loaded when command names collide", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".base-context", "extensions");
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.pi", async () => {
			const piDir = join(cwd, ".base-context");
...
Content`,
	...
	describe("bundled skills", () => {
		...
		it("should let a project skill override the bundled websearch skill", async () => {
			const projectSkillDir = join(cwd, ".base-context", "skills", "websearch");
			mkdirSync(projectSkillDir, { recursive: true });
			writeFileSync(
