...
describe("tools manager", () => {
	...
	afterEach(() => {
		...
		if (originalOffline === undefined) delete process.env.BASE_CONTEXT_OFFLINE;
		else process.env.BASE_CONTEXT_OFFLINE = originalOffline;
		rmSync(toolState.toolsDir, { recursive: true, force: true });
	});
	...
	it("formats actionable platform-specific ripgrep warnings", () => {
		...
		expect(mac).toContain("Base Context and subagents remain available");
	});
});
