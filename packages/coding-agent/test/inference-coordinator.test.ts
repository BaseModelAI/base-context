...
const transientOutput = (): ai.AssistantMessage => ({
	...output(),
	stopReason: "error",
	errorMessage: "Provider unavailable",
	diagnostics: [{ type: "provider_stream_failure", timestamp: 1, details: { kind: "server_error", status: 503 } }],
});

function fakeTransport(
	sent: () => void | Promise<void>,
	gate?: Promise<void>,
	result: () => ai.AssistantMessage = output,
): typeof ai.streamSimple {
...
describe("native inference coordination", () => {
	...
	it("recovers an auxiliary completion on its retained source and original inputs", async () => {
		...
		const bindSink = vi.fn(() => ({
			source: Promise.resolve(source("summary-source")),
			retain: () => {},
			release: releaseSink,
		...
		const requests = new InferenceCoordinator(bindSink);
		const captured = requests[captureNativeCompactionRequests]();
		let sends = 0;
		let notifyRecovery!: () => void;
		const recovering = new Promise<void>((resolve) => {
			notifyRecovery = resolve;
		});
		requests.setProviderRecoveryPolicy(() => {
			if (sends) notifyRecovery();
			return { enabled: true, baseDelayMs: 1, maxRetryDelayMs: 1 };
		});
		vi.spyOn(ai, "streamSimple").mockImplementation(
			fakeTransport(
				...
				undefined,
				() => (sends < 4 ? transientOutput() : output()),
			),
		);
		const input = structuredClone(context);
		const completion = captured.complete(model, input, undefined, { purpose: "summary" });
		try {
			await recovering;
			input.messages.length = 0;
			expect(requests.hasPending).toBe(true);
			expect(releaseSink).not.toHaveBeenCalled();
			await expect(completion).resolves.toMatchObject({ stopReason: "stop" });
			expect(sends).toBe(4);
			expect(bindSink).toHaveBeenCalledTimes(1);
			expect(new Set(facts.map((fact) => fact.operationId)).size).toBe(1);
			expect(new Set(facts.map((fact) => fact.attemptId)).size).toBe(4);
			expect(facts.filter((fact) => fact.type === "attempt_settled").map((fact) => fact.receipt.outcome)).toEqual([
				"failed",
				"failed",
				"failed",
				"completed",
			]);
			expect(vi.mocked(ai.streamSimple).mock.calls.every((call) => call[1].messages.length === 1)).toBe(true);
	...
	it("cancels auxiliary backoff without releasing ownership early or sending again", async () => {
		const releaseSink = vi.fn(async () => {});
		const requests = new InferenceCoordinator(() => ({
			source: Promise.resolve(source("cancelled-summary")),
			retain: () => {},
			release: releaseSink,
			persist: async () => {},
		}));
		...
		requests.setProviderRecoveryPolicy(() => {
			if (sends) notifyRecovery();
			return { enabled: true, baseDelayMs: 60_000, maxRetryDelayMs: 60_000 };
		});
		vi.spyOn(ai, "streamSimple").mockImplementation(
			fakeTransport(
				() => {
		...
		const completion = captured.complete(model, context, { signal: controller.signal }, { purpose: "summary" });
		try {
			await recovering;
			let idle = false;
	...
	it("charges admitted SDK retries before refusing transport and permits a fresh auxiliary invocation", async () => {
		const { manager, requests } = await auxiliaryFixture();
		requests.setProviderRecoveryPolicy(() => ({ enabled: true, baseDelayMs: 1, maxRetryDelayMs: 1 }));
		const captures: InferenceCoordinator[] = [];
		let failing = true;
	...
	it("keeps late settlement and semantic retries on their captured source and rebinds children", async () => {
		...
		requests.setProviderRecoveryPolicy(() => ({ enabled: true, baseDelayMs: 1, maxRetryDelayMs: 1 }));
		vi.mocked(ai.streamSimple).mockImplementationOnce(fakeTransport(() => {}, undefined, transientOutput));
		current = source("selected-again");
		const failedSide = await (await side(model, context)).result();
		expect(requests.hasPending).toBe(true);
		await expect(side.recoverProviderFailure!(failedSide, new AbortController().signal)).resolves.toBe(true);
