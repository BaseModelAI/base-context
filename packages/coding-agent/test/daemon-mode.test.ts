...
import { DEFAULT_NATIVE_RECOVERY_LIMITS, recoverCapturedHistory } from "../src/core/selective-recovery.js";
...
describe("daemon mode helpers", () => {
	...
	it("persists a real child completion for passive discovery, roster, and listing", async () => {
		...
		try {
			...
			const parentManager = trackSession(await SessionManager.create(tempDir, sessionDir));
			await parentManager.appendSessionInfo("parent");
			...
			const createRuntime = vi.fn(makeNativeRuntime);
			const daemon = new AgentDaemon(join(tempDir, "daemon.sock"), {
				defaultSessionConfig: { agentDir: tempDir, cwd: tempDir, sessionDir },
				createRuntime,
			});
			await openFixtureRlmJournal(daemon);
			const internals = daemon as unknown as {
				sessions: Map<string, ActiveSessionState>;
	...
	it("defers RLM heartbeats while a subagent is binding", async () => {
		...
		try {
			...
			const createRuntime = vi.fn(async (options: Parameters<CreateAgentSessionRuntimeFactory>[0]) => {
				const result = await makeNativeRuntime(options);
				const session = result.session;
				session.promptHeartbeat = promptHeartbeat;
	...
	it("skips a cron job cancelled while its prompt waits for admission", async () => {
		...
		try {
			const sessionDir = join(tempDir, "sessions");
			const manager = trackSession(await SessionManager.create(tempDir, sessionDir));
	...
	it("delivers only a child-result capsule and recovers its report after sender disposal and recipient reopen", async () => {
		const { parent, child, parentState, childState, internals, controller } = await makeAgentResultDaemonFixture();
		let reopened: SessionManager | undefined;
		try {
			const selectedLine = "  capsulereportmarker3: preserve the final newline and the original indentation.";
			const findings = `# Latest parser findings\n\n${selectedLine}\n\n${"Observed parser output, not a verified outcome.\n".repeat(200)}`;
			const reports = [
				"capsulereportmarker1: initial parser notes\n".repeat(200),
				"capsulereportmarker2: revised parser notes\n".repeat(200),
				findings,
			];
			const receipts: AgentSessionMessageReceipt[] = [];
			const providerMessages: string[] = [];
			let summary = "";
			for (const [index, report] of reports.entries()) {
				summary =
					index === 2
						? `Parser investigation complete. Supersedes reports ${receipts.map((receipt) => receipt.resultRef!.ref).join(", ")}; read only this latest report's relevant lines.`
						: `Parser investigation update ${index + 1}; details are available in the report.`;
				parent.appendResponses([
					(context) => {
						providerMessages.push(JSON.stringify(context.messages));
						return fauxAssistantMessage("I received the capsule and its report reference.");
					},
				]);
				const receipt = await controller.sendAgentMessage({
					target: parentState.activeSessionId,
					message: summary,
					findings: report,
				});
				await parent.session.waitForIdle();
				expect(receipt.resultRef).toEqual({
					sourceSessionId: parent.session.sessionId,
			...
			const reference = receipt.resultRef!;
			expect(receipt.message).toContain(summary);
			expect(receipt.message).toContain(reference.ref);
			expect(JSON.stringify(receipts)).not.toContain("capsulereportmarker");
			expect(providerMessages).toHaveLength(3);
			expect(providerMessages[2]).toContain(summary);
			expect(providerMessages.every((messages) => !messages.includes("capsulereportmarker"))).toBe(true);
			expect(JSON.stringify(parent.session.messages)).not.toContain("capsulereportmarker");
			expect(
				parent.session.messages.filter(
					(message) => message.role === "custom" && message.customType === "agent_message",
				),
			).toHaveLength(3);
			expect(parent.session.messages).toContainEqual(
				expect.objectContaining({
					...
					details: expect.objectContaining({
						...
						from: expect.objectContaining({ sessionId: child.session.sessionId }),
					}),
				}),
			);
			...
			await parent.session.disposeAsync();
			internals.sessions.delete(parentState.activeSessionId);
			reopened = await SessionManager.open(sessionFile);
			for (const earlier of receipts.slice(0, 2)) {
				expect(summary).toContain(earlier.resultRef!.ref);
				expect(await reopened.getHistoryEntry(earlier.resultRef!.ref)).toMatchObject({
					id: earlier.resultRef!.ref,
					kind: "custom",
					qualification: "native-recovery",
				});
			}
			const read = await reopened.readBranchHistory((history) =>
			...
			expect(read).toMatchObject({
				...
				authority: "tool-data",
				freshness: "unknown",
				results: [{ records: [{ ...reference, startLine: 3, endLine: 4, text: `${selectedLine}\n` }] }],
			});
			const search = await reopened.readBranchHistory((history) =>
				recoverCapturedHistory(history.branchContext, {
					action: "search",
					sourceSessionId: reference.sourceSessionId,
					query: "capsulereportmarker3",
				}),
			);
			expect(search.results.flatMap((result) => result.records)).toEqual([
				expect.objectContaining({ ...reference, startLine: 3, endLine: 3, text: selectedLine }),
			]);
			const context = buildSessionContext(await reopened.readBranch(), reopened.getLeafId());
			expect(JSON.stringify(context.messages)).toContain(summary);
			expect(JSON.stringify(context.messages)).not.toContain("capsulereportmarker");
		} finally {
	...
	it("refuses an oversized child report before notifying the recipient or returning a result reference", async () => {
		const { parent, child, parentState, controller } = await makeAgentResultDaemonFixture();
		const accept = vi.spyOn(parent.session, "acceptAgentMessagePrompt");
		try {
			const leafId = parent.sessionManager.getLeafId();
			await expect(
				controller.sendAgentMessage({
					target: parentState.activeSessionId,
					message: "This oversized report must not be delivered.",
					findings: "x".repeat(DEFAULT_NATIVE_RECOVERY_LIMITS.maxSourceBytes),
				}),
			).rejects.toThrow(/byte limit|budget/i);
			expect(accept).not.toHaveBeenCalled();
			expect(parent.sessionManager.getLeafId()).toBe(leafId);
	...
	it.each([
		...
		try {
			const recent = trackSession(await SessionManager.create(tempDir, tempDir));
			const sessionPath = await recent.materializeSessionFile();
			await recent.appendSessionInfo("Recent");
	...
	it("restores a completed subagent through its parent when an RLM heartbeat becomes due", async () => {
		...
		try {
			const sessionDir = join(tempDir, "sessions");
			const parentManager = trackSession(await SessionManager.create(tempDir, sessionDir));
			await parentManager.appendSessionState({ status: "active" });
			const parentSessionFile = parentManager.getSessionFile();
			const parentArtifactDir = parentManager.getSessionArtifactDir();
			...
			const childManager = trackSession(
				await SessionManager.create(tempDir, childSessionDir, { parentSession: parentSessionFile }),
			);
			await childManager.appendSessionInfo("heartbeat-child");
	...
	it("replaces a resident top-level RLM child when restoring its heartbeat", async () => {
		...
		try {
			const fixture = await makePersistedRlmDaemonFixture(tempDir);
			const parentManager = trackSession(await SessionManager.open(fixture.parentSessionFile));
			await parentManager.appendSessionState({ status: "active" });
			await parentManager.close();
			await fixture.openJournal();
			const internals = fixture.daemon as unknown as {
	...
	it("cancels an RLM heartbeat for a resident top-level session that is not a registered child", async () => {
		...
		try {
			...
			const sessionManager = trackSession(
				await SessionManager.create(tempDir, join(parentArtifactDir, "unregistered-child"), {
	...
	it("cancels a detached subagent heartbeat when its parent is archived", async () => {
		...
		try {
			const sessionDir = join(tempDir, "sessions");
			const parentManager = trackSession(await SessionManager.create(tempDir, sessionDir));
			await parentManager.appendSessionState({ status: "archived" });
			const parentSessionFile = parentManager.getSessionFile();
			const parentArtifactDir = parentManager.getSessionArtifactDir();
			if (!parentSessionFile || !parentArtifactDir) {
				throw new Error("Missing parent session paths");
			...
			const childManager = trackSession(
				await SessionManager.create(tempDir, join(parentArtifactDir, "child-1"), {
					parentSession: parentSessionFile,
				}),
			);
			const childSessionFile = childManager.getSessionFile();
	...
	it("lists passive descendants under a nonresident saved root", async () => {
		...
		try {
			const fixture = await makePersistedRlmDaemonFixture(tempDir);
			const parentManager = trackSession(await SessionManager.open(fixture.parentSessionFile));
			await parentManager.appendMessage({ role: "user", content: "parent task", timestamp: 0 });
	...
	it("prefers registry depth when listing a passive legacy child", async () => {
		...
		try {
			...
			await parentManager.flushNow();
			await parentManager.close();
			await rewriteSessionHeader(fixture.childSessionFile, (header) => {
				delete header.rlmDepth;
			});
...
`,
	...
	it("does not match a renamed passive child by its stale registry name", async () => {
		...
		try {
			...
			const siblingManager = trackSession(
				await SessionManager.create(tempDir, siblingSessionDir, { parentSession: fixture.parentSessionFile }),
			);
			await siblingManager.appendSessionInfo("spawn-worker");
			await siblingManager.flushNow();
			await siblingManager.close();
	...
	it("prefers the persisted header depth when a legacy registry entry lacks one", async () => {
		...
		try {
			const fixture = await makePersistedRlmDaemonFixture(tempDir, { childDepth: 2 });
		...
		try {
			...
			fixture.createRuntime.mockImplementation(async (runtimeOptions) => {
				if (runtimeOptions.sessionManager.getSessionFile() === fixture.childSessionFile) {
					...
					if (failHydration) {
						failHydration = false;
						runtimeOptions.sessionOptions?.rlmChildAdmission?.confirmUnboundCleanup();
	...
	it("repairs a wrong-kind pending open while preserving the passive row id", async () => {
		...
		try {
			...
			Object.assign(wrongState.runtime, {
				dispose: vi.fn(async () => {
					await wrongState.runtime.session.disposeAsync();
				}),
				metadata: { kind: "root", createdAt: 1 },
				session: makeRuntimeSession(await SessionManager.open(fixture.childSessionFile, fixture.childSessionDir)),
			});
		...
		try {
			...
			internals.listPassiveRlmSubagents = vi.fn(async () => {
				...
				if (passiveListCalls === 2) {
					vi.spyOn(childState.runtime.session, "isStreaming", "get").mockReturnValue(true);
					vi.spyOn(childState.runtime.session, "isSessionActive", "get").mockReturnValue(true);
				}
				return [];
			});
	...
	it("deletes a passive child without hydrating it and treats unknown children benignly", async () => {
		...
		try {
			...
			const ledgerOps = readFileSync(join(ledgerDir, ledgerFile), "utf8")
				.trim()
				.split(/\r?\n/)
				.map((line) => (JSON.parse(line) as { payload: { op: string; childId?: string } }).payload);
	...
	it("resolves a delete that joins an in-flight passivation close", async () => {
	...
	it("cancels scheduled jobs when deleting a pre-ledger legacy child without hydrating it", async () => {
		...
		try {
			const fixture = await makePersistedRlmDaemonFixture(tempDir, { legacyRegistryAfterLedgerSeed: true });
			await fixture.openJournal();
			const internals = fixture.daemon as unknown as {
				createRuntime(command: Extract<DaemonCommand, { type: "create" }>): Promise<ActiveSessionState>;
				createSubagentRuntimeHost(parent: ActiveSessionState): SubagentRuntimeHost;
			...
			const parentState = await internals.createRuntime({ type: "create", sessionPath: fixture.parentSessionFile });
			// The real owner seeded an empty ledger before the legacy registry appeared.
			// Deletion must resolve the missed child without replacing an owned journal.
			const job = internals.cronStore.create({
				activeSessionId: "gone",
				sessionId: "gone",
				sessionFile: fixture.childSessionFile,
	...
	it("gives RLM subagents messaging controllers for their own nested children", async () => {
		...
		try {
			...
			// The first parent already owns its one resident child.
			const cancellationParent = await internals.createRuntime({
				type: "create",
				sessionPath: join(tempDir, "cancellation-parent.jsonl"),
			});
			const sessionsBeforeCancelledStartup = internals.sessions.size;
			const getStatus = vi.spyOn(cancellationParent.runtime.session, "getRlmChildRunStatus");
			getStatus.mockReturnValue("cancelled");
			await expect(
				internals.createRlmSubagentRuntime(cancellationParent, {
					parentSession: cancellationParent.runtime.session,
					id: "cancelled-child",
			...
			expect(getStatus).toHaveBeenCalledWith("cancelled-child");
	...
	it("applies killed effects when kill joins a passivation close", async () => {
		...
		try {
			...
			const sessionFile = join(tempDir, "session.jsonl");
			const manager = trackSession(await SessionManager.open(sessionFile, tempDir, tempDir));
			await manager.appendMessage({ role: "user", content: "keep this session", timestamp: 1 });
			...
			expect(await readSessionState(sessionFile)).toEqual({ status: "archived" });
		} finally {
			releaseDispose();
			await closeFixtureSessions();
			rmSync(tempDir, { recursive: true, force: true });
	...
	it("applies a stronger reason after the joined close rejects", async () => {
		...
		try {
			...
			const manager = trackSession(await SessionManager.open(join(tempDir, "session.jsonl"), tempDir, tempDir));
			...
			expect(internals.cronStore.list().find((job) => job.id === cron.id)).toMatchObject({ status: "cancelled" });
			expect(await readSessionState(manager.getSessionFile()!)).toEqual({ status: "archived" });
		...
		try {
			...
			for (const state of [parent, child]) {
				const manager = trackSession(
					await SessionManager.open(join(tempDir, `${state.activeSessionId}.jsonl`), tempDir, tempDir),
				);
				await manager.appendSessionState({ status: "active" });
				await manager.close();
				state.runtime = {
			...
			writeFileSync(parent.runtime.session.sessionFile!, "broken journal\n");
			const existingClose = {
			...
			await expect(internals.closeSession(parent, "killed")).rejects.toThrow("Malformed session journal record");
			...
			expect(readFileSync(parent.runtime.session.sessionFile!, "utf8")).toBe("broken journal\n");
			expect(await readSessionState(child.runtime.session.sessionFile!)).toEqual({ status: "archived" });
			expect(existingClose.reason).toBe("killed");
			await expect(internals.closeSession(parent, "killed")).resolves.toBeUndefined();
			const view = await SessionManager.openReadOnly(child.runtime.session.sessionFile!);
			try {
				expect(
					view.getEntries().filter((entry) => entry.type === "session_state" && entry.state.status === "archived"),
				).toHaveLength(1);
			} finally {
				await view.close();
			}
	...
	it("upgrades resident descendants when kill joins a parent shutdown close", async () => {
		...
		try {
			...
			parentState.runtime.dispose = vi.fn(async () => {
				...
				await parentState.runtime.session.disposeAsync();
			});
			...
			expect(await readSessionState(fixture.childSessionFile)).toEqual({ status: "archived" });
		} finally {
			releaseParentDispose();
			await closeFixtureSessions();
			rmSync(tempDir, { recursive: true, force: true });
	...
	it("streams detached saved-session catalog requests", async () => {
		...
		try {
			const sessionDir = join(tempDir, "sessions");
			const session = trackSession(await SessionManager.create(tempDir, sessionDir));
	...
	it("capability-gates cancellation after prompt ownership", async () => {
		...
		// Current-protocol clients without cancelOwned retain pre-ownership-only cancellation.
		internals.parseCommandAndRegisterPromptAdmission(
			client,
			JSON.stringify(
				createDaemonCommandEnvelope(
...
const fixtureHarnesses = new Set<Awaited<ReturnType<typeof createHarness>>>();
const fixtureJournalDaemons = new Set<AgentDaemon>();
...
async function openFixtureRlmJournal(daemon: AgentDaemon): Promise<void> {
	fixtureJournalDaemons.add(daemon);
	await (daemon as unknown as { openRlmJournalOwner(): Promise<void> }).openRlmJournalOwner();
}

async function closeFixtureSessions(): Promise<void> {
	for (const harness of [...fixtureHarnesses].reverse()) await harness.cleanup();
	fixtureHarnesses.clear();
	for (const daemon of fixtureJournalDaemons) {
		await (daemon as unknown as { closeRlmJournal(): Promise<void> }).closeRlmJournal();
	}
	fixtureJournalDaemons.clear();
	await Promise.all([...fixtureSessions].map((manager) => manager.close()));
	fixtureSessions.clear();
}

async function rewriteSessionHeader(sessionFile: string, update: (header: SessionHeader) => void): Promise<void> {
	const records = [];
	for await (const { entry } of readSessionJournal(sessionFile)) records.push(entry);
	update(records[0] as SessionHeader);
	let cursor = INITIAL_JOURNAL_CURSOR;
	const lines = records.map((entry) => {
		const encoded = encodeJournalFrame(entry, cursor);
		cursor = encoded.next;
		return encoded.line;
	});
	writeFileSync(sessionFile, lines.join(""));
	const refreshed = await SessionManager.open(sessionFile);
	await refreshed.close();
}

async function readSessionState(sessionFile: string) {
	const view = await SessionManager.openReadOnly(sessionFile);
	try {
		return view.getSessionState();
	} finally {
		await view.close();
	}
}
...
async function makeNativeRuntime(
	...
	const harness = await createHarness({
		...
		rlmDepth: options.sessionOptions?.rlmDepth,
		rlmMaxDepth: options.sessionOptions?.rlmMaxDepth,
		rlmChildAdmission: options.sessionOptions?.rlmChildAdmission,
		requestTokenBudget: options.sessionOptions?.requestTokenBudget,
	});
	fixtureHarnesses.add(harness);
	const session = harness.session;
	vi.spyOn(session, "disposeAsync");
	vi.spyOn(session, "abort");
	vi.spyOn(session, "bindExtensions");
	vi.spyOn(session, "initialize");
	vi.spyOn(session, "setSessionName");
	vi.spyOn(session, "registerRlmChildSession");
	vi.spyOn(session, "releaseRlmChildSession");
	return {
		session,
		extensionsResult: session.resourceLoader.getExtensions(),
		services: {
			cwd: options.cwd,
...
async function makePersistedRlmDaemonFixture(
	tempDir: string,
	options: {
		legacyRegistryAfterLedgerSeed?: boolean;
		childDepth?: number;
	...
	const childManager = trackSession(
		await SessionManager.create(tempDir, childSessionDir, {
			parentSession: parentSessionFile,
			rlmDepth: options.childDepth,
		}),
	);
	await childManager.appendSessionInfo("spawn-worker");
	await childManager.appendSessionInfo("renamed-worker");
	await childManager.appendMessage({ role: "user", content: "complete this task", timestamp: 1 });
	await childManager.flushNow();
	const childSessionFile = childManager.getSessionFile();
	...
	const grandchildManager = trackSession(
		await SessionManager.create(tempDir, grandchildSessionDir, { parentSession: childSessionFile }),
	);
	await grandchildManager.appendSessionInfo("nested-worker");
	await grandchildManager.appendMessage({ role: "user", content: "complete the nested task", timestamp: 2 });
	await grandchildManager.flushNow();
	...
	await Promise.all([parentManager.close(), childManager.close(), grandchildManager.close()]);
	...
	const createRuntime = vi.fn(async (runtimeOptions: Parameters<CreateAgentSessionRuntimeFactory>[0]) => {
		...
		const result = await makeNativeRuntime(runtimeOptions);
		const runtimeSession = result.session;
		const disposeAsync = runtimeSession.disposeAsync.bind(runtimeSession);
		...
		if (isChild && options.childDisposeGate) {
			runtimeSession.disposeAsync = vi.fn(async (disposeOptions) => {
				options.childDisposeStarted?.();
				await options.childDisposeGate;
				await disposeAsync(disposeOptions);
			});
		}
		Object.defineProperty(runtimeSession, "hasPendingAdmissionWaiters", {
			get: () => admissionPending,
		});
		runtimeSession.acceptAgentMessagePrompt = acceptAgentMessagePrompt;
		return result;
	});
	const daemon = new AgentDaemon(join(tempDir, "daemon.sock"), {
		defaultSessionConfig: { agentDir: tempDir, cwd: tempDir, sessionDir },
	...
	let journalReady: Promise<void> | undefined;
	const openJournal = () => (journalReady ??= openFixtureRlmJournal(daemon));
	if (options.legacyRegistryAfterLedgerSeed) await openJournal();
	writeFileSync(
...
async function makeAgentResultDaemonFixture() {
	const parent = await createHarness({ persistSession: true, tools: [], rlmDepth: 0 });
	const childManager = await SessionManager.create(parent.tempDir, join(parent.tempDir, "children"), {
		parentSession: parent.session.sessionFile,
		rlmDepth: 1,
	});
	const child = await createHarness({ sessionManager: childManager, tools: [], rlmDepth: 1 });
	const daemon = new AgentDaemon(join(parent.tempDir, "daemon.sock"), {
		defaultSessionConfig: { agentDir: parent.tempDir, cwd: parent.tempDir },
		createRuntime: vi.fn(),
	});
	const parentState = makeState("result-parent");
	parentState.runtime = {
		...parentState.runtime,
		cwd: parent.tempDir,
		metadata: { kind: "top-level", createdAt: 1 },
	...
	const childState = makeState("result-child", parentState.activeSessionId);
	childState.runtime = {
		...childState.runtime,
		cwd: child.tempDir,
		metadata: {
			...
			parentSessionFile: parent.session.sessionFile,
		},
	...
	return {
		...
		childState,
		internals,
		controller: internals.createAgentMessageController(() => childState),
	};
}
