...
const bootstrapKernel = process.env.BASE_CONTEXT_BOOTSTRAP_KERNEL_ON_INSTALL === "1";
const bootstrapTools = process.env.BASE_CONTEXT_BOOTSTRAP_TOOLS_ON_INSTALL === "1";

if (!bootstrapKernel && !bootstrapTools) {
	process.exit(0);
}

if (bootstrapKernel && process.env.BASE_CONTEXT_INSTALL_UV === undefined) {
