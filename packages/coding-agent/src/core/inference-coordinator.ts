...
export class InferenceCoordinator {
	...
	private work = {
		...
		providerRecoveryPolicy: undefined as (() => ProviderRecoveryPolicy) | undefined,
	};

	setProviderRecoveryPolicy(policy: () => ProviderRecoveryPolicy): void {
		this.work.providerRecoveryPolicy = policy;
	}
	...
	/** Hold one logical operation through settled sends and recovery delays. */
	private retainWork(binding: SinkUse): (failures?: unknown[]) => Promise<void> {
		this.assertAdmission();
		binding.sink.retain();
		binding.release = undefined;
		binding.captures++;
		...
		return (failures = []) => {
			release ??= (async () => {
				binding.captures--;
				try {
					await this.finishSink(binding, [], false, failures);
				} finally {
					this.active.delete(pending);
					this.work.pending.delete(pending);
					finish();
	...
	private providerRecoveryWait(): (message: AssistantMessage, signal?: AbortSignal) => Promise<boolean> {
		...
		return async (message, callerSignal) => {
			...
			const delayMs = Math.min(nextDelayMs ?? policy.baseDelayMs, policy.maxRetryDelayMs);
	...
	private async completeWithProviderRecovery(
		model: Model<Api>,
		context: Context,
		options: SimpleStreamOptions | undefined,
		request: InferenceRequestOptions,
	): Promise<InferenceSettlement> {
		// Retry the same request, not a caller's later mutable inputs. Keep callbacks/signals live.
		model = structuredClone(model);
		context = structuredClone({
			...context,
			...(context.tools
		...
		const semanticEdgeId = request.semanticEdgeId ?? options?.headers?.[MODEL_REQUEST_ID_HEADER];
		const operationId = request.operationId ?? semanticEdgeId ?? randomUUID();
		const operation = this.bindOperation(model, { ...request, semanticEdgeId }, operationId);
		const release = this.retainWork(operation.binding);
		const recover = this.providerRecoveryWait();
		let failure: unknown;
		try {
			while (true) {
				const settlement = await (await this.execute(operation, model, context, options, streamSimple)).settled;
				// Rejected execution/settlement, including local allowance exhaustion, never reaches recovery.
				if (!(await recover(settlement.message, options?.signal))) return settlement;
			}
		} catch (error) {
			failure = error;
			throw error;
		} finally {
			await release(failure === undefined ? [] : [failure]);
		}
	}
	...
	complete(
		...
		completion = (async () => {
			const settlement = this.work.providerRecoveryPolicy?.().enabled
				? await this.completeWithProviderRecovery(model, context, options, request)
				: await (await this.start(model, context, options, request)).settled;
	...
	bindAuxiliaryStream(streamFn: StreamFn, request: InferenceRequestOptions): DisposableInferenceStream {
		const captured = this.capture();
		const release = captured.retainWork(captured.capturedSink!);
		const recover = captured.providerRecoveryWait();
		const stream = captured.bindStream(streamFn, request);
		let disposal: Promise<void> | undefined;
		return Object.assign(stream, {
			recoverProviderFailure: async (message: AssistantMessage, signal?: AbortSignal) => {
				if (!(await recover(message, signal))) return false;
				captured.prepareTurnRetry();
				return true;
			},
			dispose: () => {
				disposal ??= (async () => {
...
export type DisposableInferenceStream = StreamFn & {
	dispose(): Promise<void>;
...
export function bindAuxiliaryInferenceStream(
	...
	if (binding) return binding.coordinator.bindAuxiliaryStream(binding.inner, request);
	const stream: StreamFn = (model, context, options) => inner(model, context, options);
	return Object.assign(stream, { dispose: async () => {} });
}
