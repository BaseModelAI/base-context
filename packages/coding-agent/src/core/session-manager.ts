...
import { AGENT_RESULT_CUSTOM_TYPE, type AgentResultData } from "./agent-results.js";
...
export class SessionManager {
	...
	[bindNativeEntryWriter](): NativeEntryWriter {
		...
		return {
			captureAgentResult: () => async (data) => {
				assertWriter();
				if (!this.isPersisted() || !this.getSessionFile() || !this.supportsCapturedHistoryReads())
					throw new Error("Agent results require an owned persistent session source");
				const entry: CustomEntry<AgentResultData> = {
					type: "custom",
					customType: AGENT_RESULT_CUSTOM_TYPE,
					data,
					id: "",
					parentId: this.leafId,
					timestamp: new Date().toISOString(),
				...
				await this._appendEntry(
					withEntryRetention(entry, undefined, "native-recovery"),
					false,
					(snapshot) => {
						const cursor = state.owner!.getSnapshot();
						// Recovery charges the entire encoded frame, including its native qualifier.
						encodeJournalFrame(
							snapshot,
							{ sequence: cursor.nextSequence, checksum: cursor.checksum },
							DEFAULT_NATIVE_RECOVERY_LIMITS.maxSourceBytes,
							undefined,
							"native-recovery",
						);
					},
