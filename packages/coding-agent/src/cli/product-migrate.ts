...
const COVERAGE = [
	...
	"Use BASE_CONTEXT_HOME=<destination> base-context schedule list --offline to list metadata without a daemon. After opening the imported session, use base-context schedule resume <new-job-id> to explicitly activate future generic recurring cron scheduling. One-shot jobs require separate explicit rescheduling, and heartbeat resume remains an explicit existing runtime action from the newly bound session using the new job ID; old job handles and kernel state are not restored.",
