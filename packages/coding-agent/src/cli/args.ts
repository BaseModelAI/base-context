...
/** Free-form objectives and commands may start with a short-option-looking token. */
const FREEFORM_VALUE_FLAGS = new Set(["--goal", "--autonomous-gate"]);
/** Prompt text may start with long-option-looking text, such as YAML frontmatter. */
...
function hasRequiredOptionValue(args: string[], index: number, flag: string, result: Args): boolean {
	...
	// Keep the standalone end-of-options delimiter for the main loop in every case.
