...
A Python-backed skill is a regular markdown skill that also ships a Python package. Base Context installs the package editable into the kernel venv (the selected release-local `runtime` for an owned installation; otherwise `~/.base-context/runtime`, under `BASE_CONTEXT_HOME` when configured; `BASE_CONTEXT_KERNEL_VENV` to override) and exposes it in the persistent Python kernel, so the agent can call it directly instead of shelling out.
...
- `rlm` and `tyro` are already present in the kernel venv. Do **not** declare `base-context-runtime` as a dependency: it is bundled with Base Context and is not currently published on PyPI. Keep the Python import and CLI target as `rlm` and `rlm.skill:cli`. The CLI entry point only works where the runtime is installed, i.e. inside the kernel venv.
...
- Declare every third-party package `run()` imports in `dependencies` — `pyproject.toml` is the source of truth. The one exception is the bundled `base-context-runtime` (see above).
...
- If the user sets `BASE_CONTEXT_KERNEL_PYTHON`, Base Context installs nothing into that environment. It must already contain the current `base-context-runtime` and default runtime packages. Skills whose imports are missing there are disabled with a warning. This override is outside the owned installation’s default CLI/Python pairing.
