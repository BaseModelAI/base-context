> Base Context can create themes. Ask it to build one for your setup.
...
Base Context loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.base-context/themes/*.json`
- Project: `.base-context/themes/*.json`
...
On first run, Base Context detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.base-context/themes
vim ~/.base-context/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/BaseModelAI/base-context/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, Base Context reloads it automatically for immediate visual feedback.
...
Base Context uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Base Context falls back to the nearest approximation.
