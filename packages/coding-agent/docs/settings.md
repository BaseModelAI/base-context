...
```json
{
  ...
  "tarball": "releases/v1.0.0/base-context-1.0.0.tgz"
}
```
...
also retain their source and pending-work ownership during recovery.
...
| `transport` | string | `"sse"` | Preferred transport for providers that support multiple transports: `"sse"`, `"websocket"`, `"websocket-cached"`, or `"auto"`. Auto is opt-in: it prefers cached WebSocket continuation where supported and allows eligible pre-stream SSE fallback. Explicit selections remain unchanged. |
...
| `npmCommand` | string[] | - | Command argv used for npm package lookup/install operations (e.g., `["mise", "exec", "node@22", "--", "npm"]`) |
...
| `inactivePackages` | array | `[]` | Imported declarations; never activated by discovery. Review and explicitly install to activate |
| `mcpServers` | object | - | Named MCP connection definitions; see [MCP integrations](mcp-integrations.md) |
...
| Setting | Type | Default | Description |
| --- | --- | --- | --- |
| `autoRefine.enabled` | boolean | `true` | Enable automatic harness refinement |
| `autoRefine.turnInterval` | number | `25` | Assistant-turn interval used by automatic refinement |
| `autoRefine.compact` | boolean | `true` | Enable the automatic refinement trigger associated with compaction |
| `autoRefine.cooldownMs` | number | `1200000` | Minimum cooldown between automatic refinements, in milliseconds |
| `autoRefine.model` | object | Main model, effort omitted | Explicit `provider`, `modelId`, and `thinkingLevel` for learning |

Refinement changes supplemental harness state, not the immutable base system prompt. `context.mode="off"` blocks new refinement even when automatic refinement is enabled. Manual `/refine` and automatic triggers use the configured learning model described below.
