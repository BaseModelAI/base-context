...
Connect external services (Linear, Notion, …) to Base Context over the
[Model Context Protocol](https://modelcontextprotocol.io).

Consistent with Base Context's programmatic RLM design, MCP integrations are **not**
...
Credentials are stored once in `~/.base-context/auth.json` under `mcp:<name>`.
...
agent) or the TUI. Both surfaces update only `~/.base-context/settings.json`:

```bash
base-context mcp add remote --url https://mcp.example.com/mcp --bearer-token-env-var EXAMPLE_TOKEN
base-context mcp add local --cwd /absolute/path --env TOKEN=EXAMPLE_TOKEN -- node server.js --stdio
base-context mcp list
base-context mcp get remote
base-context mcp remove remote
```
...
are environment-variable references. Project `.base-context/settings.json` MCP
...
- Generic MCP connections are kernel-local. Separate Base Context sessions use
  separate connections even when they reference the same user setting.
- A custom `BASE_CONTEXT_KERNEL_PYTHON` must include the current
  `base-context-runtime` dependencies.
