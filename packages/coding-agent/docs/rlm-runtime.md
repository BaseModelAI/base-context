...
Base Context gives each agent session a persistent Python REPL kernel and a native recursive sub-agent interface. The Python `rlm` package is a model-facing shim; the TypeScript host owns child execution, persistence, usage accounting, and lifecycle.
...
flowchart TD
    ...
    runtime["base-context-runtime<br/>rlm module + Python skills"]
...
The bundled `base-context-runtime` distribution exports the `rlm` Python interface:
...
The admission handle does not contain usage or completion data. Base Context asynchronously folds the child's assistant usage and cost into the parent assistant turn that launched it.
...
Session-local state lives in the session artifact directory under `harness/harness_state.json`. Explicitly global entries live under `~/.base-context/harness/`. The Python store reloads after external modification so host-side `/refine` writes and kernel writes do not overwrite each other.
...
The `.jsonl` session names above contain native framed records, not plain JSONL. Exact artifact files are created only when their features are used. Non-persistent sessions place RLM directories under the OS temporary directory and do not gain revivable session artifacts.
...
| Failure | Behavior |
|---|---|
| Managed runtime is missing | Kernel bootstrap rebuilds it; a custom `BASE_CONTEXT_KERNEL_PYTHON` without a current `base-context-runtime` is rejected at kernel startup. |
