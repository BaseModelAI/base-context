...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. Base Context waits for the factory before startup continues, so the provider is available during interactive startup and to `base-context model list`.
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map Base Context levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@ponythewhite/base-context-ai";
...
Credentials are persisted in `~/.base-context/auth.json`:
...
interface ProviderModelConfig {
  ...
  /** Maps Base Context thinking levels to provider/model-specific values; null marks a level unsupported. */
