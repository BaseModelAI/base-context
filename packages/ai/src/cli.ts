...
async function login(providerId: OAuthProviderId): Promise<void> {
	...
	try {
		const credentials = await provider.login({
			...
			onManualCodeInput: () =>
				promptFn("Paste the authorization code or full redirect URL (or finish in the browser):"),
		});
...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: base-context-ai <command> [provider]

Commands:
  login [provider]  Log in to an OAuth provider
...
Examples:
  base-context-ai login              # interactive provider selection
  base-context-ai login anthropic    # login to specific provider
  base-context-ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'base-context-ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'base-context-ai --help' for usage`);
	process.exit(1);
}
