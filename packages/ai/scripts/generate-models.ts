...
async function generateModels() {
	if (process.argv.includes("--offline-snapshot")) {
		// Re-emit committed metadata without discovery, credentials, or unrelated model updates.
		const models = Object.values(EXISTING_MODELS).flatMap((models) => Object.values(models)) as Model<Api>[];
		console.log("Using the existing offline catalog snapshot; no metadata was fetched.");
		writeModels(models);
		return;
	}
